...
static ZH_ENTRIES: &[(&str, &str)] = &[
    ...
    ("generative fill via gpt-image… (high quality can run minutes — the window stays busy until it lands)",
        "gpt-image 生成填充中…（高质量可能需要数分钟——完成前窗口保持忙碌）"),
    ...
    ("AI generating… (gpt-image; high quality can run minutes — the window stays busy until it lands; hi-res input needs a full-frame develop first)",
        "AI 生成出片中…（gpt-image；高质量可能需要数分钟——完成前窗口保持忙碌；高分辨率输入需先全幅显影）"),
    ...
    ("Colour range — click the colour to pick in the image · Esc to exit",
