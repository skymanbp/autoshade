...
fn scratch(tag: &str) -> (std::path::PathBuf, std::path::PathBuf, std::path::PathBuf) {
    let dir = std::env::temp_dir().join(format!("autoshop-repro-{tag}"));
...
fn write_save(raw: &std::path::Path, r: &EditRecipe) {
    std::fs::write(autoshop::store::recipe_target(raw), serde_json::to_string_pretty(r).unwrap())
        .unwrap();
}

fn name_of(raw: &std::path::Path, n: u32) -> Option<String> {
    autoshop::store::read_version_meta(raw).into_iter().find(|e| e.n == n).and_then(|e| e.name)
}
...
fn a_version_name_dies_with_its_number_and_is_never_inherited() {
    ...
    write_save(&raw, &b);
    let v1 = autoshop::store::read_version_meta(&raw)
        .into_iter()
        .find(|e| e.n == 1)
        .expect("the gate records the snapshot it took");
    assert_eq!(v1.origin.as_deref(), Some(autoshop::store::VERSION_ORIGIN_AUTO));
    ...
    autoshop::store::set_version_name(&raw, 1, Some("日落偏暖")).unwrap();
    assert_eq!(name_of(&raw, 1).as_deref(), Some("日落偏暖"));
    assert_eq!(
        autoshop::store::read_version_meta(&raw)[0].origin.as_deref(),
        Some(autoshop::store::VERSION_ORIGIN_AUTO),
    ...
    assert!(
        autoshop::store::read_version_meta(&raw).is_empty(),
    ...
    assert_eq!(autoshop::store::backup_saved_develop(&raw, Some(&c)).unwrap(), Some(2));
    ...
    autoshop::store::record_version_meta(
        &raw,
    ...
    .unwrap();
    autoshop::store::set_version_name(&raw, 2, Some("  暖调  ")).unwrap();
    let v2 = autoshop::store::read_version_meta(&raw).into_iter().find(|e| e.n == 2).unwrap();
    ...
    assert_eq!(v2.origin.as_deref(), Some(autoshop::store::VERSION_ORIGIN_USER));

    // Clearing a name leaves the provenance standing (the two are separate
    // facts, and only one of them is the user's to type).
    autoshop::store::set_version_name(&raw, 2, None).unwrap();
    let v2 = autoshop::store::read_version_meta(&raw).into_iter().find(|e| e.n == 2).unwrap();
...
fn the_version_name_sidecar_is_advisory_and_non_generational() {
    ...
    assert_eq!(
        autoshop::store::backup_saved_develop(
            &raw,
    ...
    autoshop::store::set_version_name(&raw, 1, Some("keep me")).unwrap();
    assert!(meta.exists());
    ...
    autoshop::store::set_version_name(&raw, 1, Some("renamed once")).unwrap();
    assert!(
        !dev.join(".version-meta.json.bak").exists(),
        "the advisory sidecar must never leave a retired generation behind"
    );
    autoshop::store::recover_orphan_baks(&raw).unwrap();
    ...
    assert_eq!(
        autoshop::store::list_versions(&raw),
        vec![1],
    ...
    assert!(autoshop::store::read_version_meta(&raw).is_empty());
    autoshop::store::set_version_name(&raw, 1, Some("after the corruption")).unwrap();
