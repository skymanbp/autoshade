# ROADMAP 历史归档（追加式，逐字迁移自 docs/ROADMAP.md）

> 2026-08-23 拆分（用户拍板）：主文件只留近期「当前状态」条目与常驻参考，
> 历史条目与已完结章节逐字迁入本文件。本文件为追加式档案：新归档条目
> 续接在对应章节，禁止改写既有内容。当前权威台账仍是 docs/ROADMAP.md。
...
- **🔒 v1.0.0 四项总纲用户下达（2026-08-22，原文拍板，接替 f725d49
  的「v0.36.0 等⑦④」——版本目标升为 v1.0.0，⑦④ 仍为发版前置且
  ⑦ 分析批已在飞）**：①**全量文本彻查**——仓库全部文本文件反漂移
  扫（README/ARCHITECTURE/ROADMAP + 盘上 M1/V2_PLAN + 模板 +
  check_docs 上门）；②**README 结构化项目说明书重写**＋效果展示
  （「AI 整图生成+AI 反推」前后对比图等，把牛逼好用之处展示出来）；
  ③**发布 v1.0.0 完整版本**＋**安装包**入 release 资产（技术选型
  已定=Inno Setup 6，ISCC.exe 本机在位亲验）；④**官方网站**——用户
  去 Cloudflare 注册域名、我方建站（形态=repo 内静态站接 Cloudflare
  Pages，域名到手后接）。**同日事故根修**：桌面 Autoshop 快捷方式
  失效，根因=R29 磁盘清理 `cargo clean` 删除 `target\release\` 而
  快捷方式指向易失构建目录；修=release 重建（在飞）+ exe 落稳定
  `dist\`（gitignore 随首笔补）+ 快捷方式重指，v1.0.0 安装包将把
  安装位正式化。批序=⑦判读（在飞）/④（待用户）→ 展示素材 →
  README 重写 → 全量彻查 → v1.0.0 发版链+安装包 → 官网。
- **R30 B4③ 训练集条款审计收官=初判确认（2026-08-22，`70832eb`，
  ...
  **R30 至此 B1/B2/B3/B4 四批全落**；余⑦（需用户新 LR 实验）④（需
  用户 GUI 目检）素材依赖。**用户拍板（2026-08-22，AskUserQuestion
  记录在案）：v0.36.0 等⑦④做完再发**——两项素材回来、⑦如需接线则
  实现后一起进发版链，发版说明一次讲全；⑦④ 指令包（镜头校正开关
  三蒙版对照实验 armA/armB + GUI 预览↔导出 100% 截图对比）已于同日
  邮寄用户 skyman.bp 邮箱。
...
- **R30 B2 MaskBrushTable 守卫导入已落地（2026-08-22，feat `2cb59a5`，
  ...
  复测并非静默丢失：重写样本 `P06` 有 1 个 correction、`P04`
...
- **R30 B2 三级证伪收官=CONFIRM + eCO 申报已提交（2026-08-22，Codex
  ...
  `recon_maskbrush3.py`）**。素材=样本二 `P04` 用户同流程重导：
  xmp 311,978→116,827B、文本 Paint 84→8、`.acr` 226,088→243,360B、
  **三个** `MaskBrushTable` token（UncompressedBytes=24,164/2,574/
  5,993）。**裁决=CONFIRM**：二级文法零改动闭合全部三表——76 二进制
  记录按序映射 76 个被转换文本 Paint、九字段+全 token 逐一相等、结构
  覆盖 32,731/32,731=100%；容器形状不变（目录 5→8 项、13 项键
  =MD5(blob) 全验、旧五 blob 字节不动、17,272B 增量精确闭合）；信封恒
  `(4,1,64000,余 Brotli 字节)`。**新证据**：多聚合各自独立成表
  （54/4/18 记录三表各归属其 Aggregate、记录序=聚合内序）；MaskValue
  非 1 值确认百万分度（51402/107477/439252）；表级 4B 字四观测对象
  恒=1（版本字判读 plausible 仍 unestablished→读侧 require==1 否则
  拒收）。**h opcode 判定**：原文 4 个 `h 1.0000` 全在幸存手势 Paint
  上未被转换（4→4 字节不动），二进制 h 形不存在亦不可推断=拒收臂
  维持、禁止猜测映射。**任务书勘误**：观测笔刷对象共 4 非 5（样本二
  原 `.acr` 只有 TIFF 蒙版对象、无笔刷 blob）。**幸存 census 修正
  二级假设**：8 幸存文本 Paint 仅 4 个 gesture 所有、另 4 个为两聚合
  所有→读侧必须**保留全部残留文本 Paint 不分归属**；手势 Paint 重写
  唯一属性变化=+`BrushGestureInterpretation="0"`。**主审第一方抽验**
  （`verify3_spot.py`）：8 目录项 MD5 全匹配、信封 (4,1,64000,1397)
  +Brotli→恰 2,574B、表字=1、T2/1 记录 sid/MaskValue=51402/首坐标
  (404621,692602) 对文本 0.051402 / `d 0.404621 0.692602` 全等、
  h 4→4。**实现文法定稿**（二级 9 具名拒收臂上加钉）：多引用独立
  解析各归属其聚合；表字 ≠1、ID 长 ≠32、未知字段枚举/opcode、计数或
  框架不一致、尾部余字节一律具名拒收。**里程碑**：eCO 版权申报
  （标题 skymanbp's Autoshop、Standard Application、v0.33.0 交存
  51 页 PDF、家庭住址）用户已于本日提交。**勘误**：上条 B2 落账时
  误吞下条 B1 的条目头行（B1 内容曾无头衔接于 B2 尾部），本笔恢复。
  下步=B2 实现批（Codex）→B3①→B4③；⑦④ 素材依赖。
- **R30 B2 格式侦查两级收官 + 主审裁决（2026-08-22，Codex ×2 分析批，
  零仓库改动；报告
  `~/.claude/plans/r30-materials/mb-format-report{,2}.md` + 素材真值对
  `mb-sample/`）**。素材=用户 3 分钟重导（`P06` 桌面副本改名破
  ...
  不外推。**实现前置**：样本二（`P04`，84 笔刷+8 AI 蒙版压力例）
  已备桌面待用户同流程重导——扩观测文法（h/f opcode、多聚合、大表）
  后开实现批。旧 `.acr`（2024）无笔刷对象=该编码确系新 LR 时代产物。
...
- **M-C 同版本重复跑落账（2026-08-22，拍板②兑现；Codex 配对分析
  ...
  抽验逐字命中）**。协议=同一颗发布位 exe（sha256 `e48f6bbc…` 亲验）、
  同 dir、同 proposer/analysis=gpt-5.6-sol、全新 state、两次续传收到
  0 fallback（转录 `mc-eval-147-v0350-repeat1{,-resume1,-resume2}.txt`）。
  **头条：gap 16.3%（精确 16.322）vs 基线 17.7%（16.661）——同版本纯
  重采样摆动 1.4pt，与跨版 +1.7pt 同量级**，且 16.3 与 v0.34.0 的 16.0
  仅差 0.3pt：「17.7% 非回归」从推断升级为直接实验证据。五项细账：
  **①midtone_hue 归因精化（接替审计初判的「抽样噪声」表述）**：该 n=1
  行在重复跑**复现**（MAE 156→163，bias 同号，且两次都是同一张
  `P02.ARW`，state 亲验）＝**可复现的单照片分歧**（AI 在这张上
  稳定提出与用户差 ~160° 的中间调色相），不是一次性偶然；v0.34 n=0
  的跨版激活差异仍未定因。**②配对统计**：147 张逐照片配对 delta 均值
  −1.339pt、两跑相关 0.989、配对 SE 0.509pt（独立性假设下 −1.34pt
  ≈2.6×SE=两跑差异不全是照片级噪声，报告如实并报）。**③低 n 不稳定性
  量化**：n<20 行 union 13 个，其贡献摆动 0.0983pt/行 vs n≥20 行的
  0.0116pt/行＝**8.48× 更不稳定**——R30⑪ 的 n<20 标注阈值获配对数据
  支持，无证据需要改。**④故障普查**：基线 1 起 vs 重复跑 11 起
  proposer 失败（10 起 `json: trailing characters` 畸形 JSON+1 起
  stream_read_error，中转当晚更不稳；无照片两跑皆败；续传协议全部
  治愈至 0 fallback）。**⑤sharpening bias 观察**：+2.71→+3.99（两次
  n=147 全量行），与 v0.34 时代 +0.44 的落差与 verifier/时段混杂，
  不能定因，仅登记。**M-C 表新增伴随行：v0.35.0 重复跑 16.3%**；判读
  维持关闭。**重锚义务**：宣传素材 gap 句一律双数并报（17.7/16.3+
  ~1.4pt 摆动幅度），Task B 已带此指令。

- **R30 已锁定（🔒 用户三拍板 2026-08-22，AskUserQuestion 记录在案）：
  全池 11 项开工 + M-C 重复跑已购在跑 + 宣传逐字批走对话分批**。
  **拍板①**：R30 候选池 11 项（①-⑪，清单见 v0.35.0 docs sweep 条的
  候选池段与 Codex 审阅裁决条）全池开工，正确性风险项先行、素材依赖
  项排后。**拍板②**：M-C 同版本重复跑付费归因批准——同一颗发布位
  exe（sha256 `e48f6bbc…` 亲验=资产表 cli 逐位一致）、同 dir
  `D:/Photography/Raw`、同 proposer/analysis=gpt-5.6-sol、独立
  state `eval-state/mc-147-v0350-repeat1.jsonl`（`--state` 新路径，
  基线状态文件零触碰）、转录
  `~/.claude/plans/r29-materials/mc-eval-147-v0350-repeat1.txt`；
  产出=同版本抽样噪声幅度 + verifier 指纹的唯一量化手段（R30⑪ 注记
  兑现）。**拍板③**：宣传 Wave 1 逐字批=主审整理后分批发对话，用户
  逐批决定；**前置发现=全部 wave-0 素材仍锚 v0.34.0/15.5%**
  （show-hn-draft.md:96-97 等），须先全量重锚 v0.35.0/17.7%（含低 n
  披露句）再送批——重锚属成规模改写，派 Codex，排在 eval 重复跑完成
  之后（避免与付费轮抢中转容量）。
  **批次规划（主审设计，目标版本 v0.36.0）**：
  **B1（零素材，码+文档）**=⑤ store `*_in` 孪生 + ⑥ census 重溯源 +
  ⑧ BODY_CAP 对称 + ⑨ 529 计费句 provider 注记 + ⑩ recovered 措辞 +
  ⑪ eval 低 n 稳健化——⑪ 设计定案：**n<20 行加标注 + 增设 n 加权
  补充聚合行，头条 gap 定义不动**（保跨版可比性——本轮 +1.7pt 虚惊
  的教训是可比性断裂的代价，不再自造一次；标注阈值论据待重复跑数据）。
  **B2**=② MaskBrushTable 格式侦查→读侧纳管或具名拒收（素材=被现代
  LR 重写过的 sidecar 一份，me5 原件未留盘须用户重导，~5 分钟同 me5
  流程）。**B3**=① gesture dabs→SAM 提示点/框（me5 定案在案零新素材，
  分割输入保真，AI 蒙版换代与否随批设计定）。**B4**=③ ADE20K/SA-1B
  训练集条款审计（Codex 研究+主审裁决，C5 §7.6 范围外项闭合）。
  **素材依赖项**：⑦ lr_mask_unwarp_norm 接线需新 LR 实验、④ GUI
  预览↔导出实拍复核需用户跑 GUI（禁 GUI 令唯用户可解）——素材指令包
  （②重导+⑦实验+④复核）随 B1 进展下发用户。
  gpt-5.6-sol xhigh 深审 19.3 万 tok + 主审逐项验算，
  `~/.claude/plans/r29-materials/codex-mc-audit-v0350.txt`）**。
  **①评价系统定性**：gap=「条件化 XMP 参数接近度」——活跃控制行的
  归一化 MAE 与曲线 RMSE **等权平均**（`eval.rs:1471-1503`），行仅在
  任一侧移动时激活（`:1182-1194`），蒙版只计数不进分。**适合**：固定
  协议下的版本回归、方向性 bias、提示词调参；**不适合**：视觉好坏、
  跨摄影师泛化、蒙版质量、低 n 行结论、跨 verifier 直比。**已知度量
  偏置具名**：等权让 n=1 行与 n=147 行同权；`temperature_k` 的
  ai_used 判定不对称（显式给值即算，`:1176-1180`）；纯 omission 行
  永不进 gap（`:1219-1225`）；曲线一条 256 点 LUT 压成一个等权槽位。
  **②147 张统计效力（主审验算与 Codex 双算一致）**：v0.35 活跃标量
  52 行，**n<20 的 13 行（25%）贡献标量 gap 的 54%**——总分对低 n 行
  极敏感。**③+1.71pt 分解定案**：`color_grade.midtone_hue` 从 n=0
  变 n=1（MAE=156，转录 :198，v0.34 无此行）**单行独占 +1.55pt**；
  全部低 n 行合计 +1.78pt、分母 55→56 抵消 −0.28pt；高 n 色调行
  混合方向（whites MAE 反而 20.57→19.41 改善）、曲线蒙版全稳。
  **无「更保守 verifier」指纹**（被点名轴不集中、HSL 稀有行反而最大
  =抽样噪声形态）；唯一弱候选=`sharpening` bias +0.44→+2.71
  （n=147，孤例登记不定性）。**④收益得失终判**：R29 七项渲染硬变更
  的收益证据是各批自己的 LR 锚定直测（径向 LUT rms≤0.0009、笔刷
  κ=0.1284、质感 45 锚点、半像素两底片、蒙版帧 0.09-0.30px vs
  8-24px——全在各批台账），**M-C 的度量域根本不含这些**；代价=七项
  迁移义务已在发版说明置顶。**17.7% 不构成回归证据，v0.35.0 判读
  关闭**。**审计产出的改进登记 →R30⑪**：eval 报告对 n<20 行加
  标注或按 n 加权的稳健性改造（等权+低 n 敏感是本次 +1.7pt 虚惊的
  机制根源）；重复跑/逐照片配对数据是 verifier 归因的唯一手段（费用
  待批，不自动跑）。
...
- **v0.35.0 两轮付费判分收官（2026-08-21 夜，拍板八兑现，发版链最后
  ...
  **gap 17.7%**（vs v0.34.0 的 16.0%）；whites mean|Δ| 19.41 / bias
  ...
  1.99 vs 用户 2.30 张/照持平。**⚠ 跨版可比性受限——但归因要写对
  （2026-08-22 主审勘误，接替昨夜初稿的错误归因）**：eval **不渲染
  像素**（`judge=false` 无视觉环、`style=0` 无检索、proposer 输入=
  相机内嵌预览+直方图），gap=提案滑杆值 vs 用户 XMP 滑杆值的纯数值
  比较——所以 **R29 七项渲染硬变更不可能移动 gap**（初稿说「本来就
  该动渲染面数字」是错的；亲证=`git diff v0.34.0..v0.35.0` 下
  `openai.rs`/`catalogue.rs` **零改动**＝proposer 提示词与量表逐字
  同，`advisor/mod.rs` +714 行全是 524/529 传输漏斗）。能动 gap 的
  只有三个：(a) **verifier 换了模型**——原
  ...
  合法 JSON 后全量）——verifier 的 verify→revise 环（≤2 轮）直接改
  提案数值，换 verifier=换这只手；(b) proposer 采样噪声（temperature
  非 0 的跨跑抖动，v0.34.0 两次尝试间同样存在）；(c) R28 5b 四条
  `*_hue` 行口径（已自报不可比）。**判读结论：17.7% vs 16.0% 的
  +1.7pt 是 verifier 效应+采样噪声，与 R29 渲染工作无关，不构成对
  v0.35.0 任何渲染变更的回归证据**。跑中经历：中途 3 张 verifier 畸形 JSON 废
...
- **v0.35.0 已发布（2026-08-21，tag → `e75f728`，发布链收官）**。
  ...
  **← 两件已于 2026-08-21 夜同窗完成（见上方判分收官条）**。GUI smoke
  与宣传批逐字批仍在用户侧。零 recipe schema 迁移义务=v0.34→v0.35
  单向断裂已在七项清单第 1 条披露。
...
- **R29 全量文档 sweep 批已落地（2026-08-21，未发版，本批）：版本推到
  ...
  **除 `Cargo.toml`/`Cargo.lock` 版本号外零行为变更**（自证：`git diff src/`
  逐行全是注释与文档字符串）。落点：**①版本** `Cargo.toml` 0.34.0→0.35.0、
  `Cargo.lock` 随 `cargo check` 自更（无测试断言 crate 版本串——`eval.rs`
  里的 `"0.34.0"` 是构造夹具值，电池全绿亲证）；**②README** 逐条失真改真
  （笔刷「rendered nowhere / `BrushCarried` / κ=0.1219 / 无闭式 / 帧不可复现」
  五条全是 v0.33 遗留、`.lcp` 从「候选」改「已落地」、八列表→290×11、
  `d_out 1.43±0.015`→√2、「every rung 一个长宽比」补 held-out 1.2 例外、
  悬空的「the table above」修掉、X-Trans 16/36 与 8+8/36 两处口径统一）＋
  **eval `--fresh`/`--state` 两旗补文档**（R28 登记的欠账，本批清）＋
  **M-C 基线改真**（README 还停在 v0.33.0 的 15.5%／whites +16.20／blacks
  −10.87，实为 v0.34.0 重基线 16.0%／+16.12／−10.60）＋七条新 Honest-scope
  条目（见下清单）＋回退分割器自愈行入诚实模型表；**③ARCHITECTURE** 状态行
  改 v0.35.0/R29、§5 笔刷段那整块「the arm still does not render」历史文本
  加分层横幅并逐条 strikethrough（κ 0.1219 标出被 0.1284 接替、两个 blocker
  各注关闭、`BrushCarried` 注明已改名）、镜头档案「cannot reproduce today」
  子句关闭、**新增两条**（B3 逐蒙版类型帧决策＋C2 像素中心约定，此前
  ARCHITECTURE 对这两项零覆盖）、电池数字刷 816/14/132/2+2；**④V2_PLAN**
  §7 item 1 折叠分支归因改真（代数恒等，非「需近正圆探针」）＋me5 第三几何
  注记、item 11 三口（负半支／全帧扇区／Midpoint 三点）关闭、item 13 加
  `MaskBrushTable` 前向登记、item 14 加 `WholeImageArea` 三条改真＋gesture
  机制定案；**⑤代码注释五处**（`render.rs` Radial 臂 Roundness 作用域两句
  ＋同族测试注释、`recipe.rs` gesture 登记二次改写、`recipe.rs`
  `WholeImageArea` 改真、`xmp.rs` census 数字待溯源锚点、`audit_i18n.py`
  一句语序错乱修通顺）。门＝`cargo check` 绿 + **816(9i)/14/132/2+2** 双配置
  全绿（与插列批基线同数＝零测试集差）+ **check_docs --gates 22/22 PASS**
  （版本抄本、电池数字、bug 模板下拉三类各处随版本号联动改真）+ fonts
  803/803 + audit_i18n 0。**M1_PLAN 改一处**（§9.4 的 M-C 参照还停在 R27 的
  15.5%，补 v0.34.0 重基线接替层 + v0.35.0 重基线是发版义务）。

  **⚠ 本批改了一条文档门，理由必须留档**：`check_docs` 的
  「download link URL + release-table + gates + zoo」行原来把 `9/9 at
  v<版本>` 也钉在 `Cargo.toml` 版本上——但**版本号是文档抄本、zoo 计数是
  实测结果**，两者钉在一起等于「版本一 bump，README 就自动宣称一个还没跑
  的门数字」，恰是这个文件存在的目的反着用。处置＝该行去掉 `9/9 at v`
  一支（其余两支照钉，行改名「download link URL + release-gates line」，
  代码里写清缘由），README/ARCHITECTURE 的 zoo 计数改成不带版本号的
  「上一次发版 9/9，每版重跑重记」。~~**发版链据此欠三件**~~ **← 三件已
  全清（2026-08-21 发版时）**：zoo 9/9 + M-B 7 sidecars/42 masks + LR
  探针都在 tag 位 `e75f728` 实跑（9 passed/0 failed，release+gui）并记
  进发布台账；README 资产表已换 v0.35.0 验证后数字（`aa3e1f5`）。

  **⚠ v0.35.0 发版说明置顶义务清单（终稿，七项硬变更）**——编号沿用拍板
  顺序，每项一句用户可读影响 + 旧配方行为：

  1. **`LensProfile` schema 硬断裂**（B3，`95c2705`）。`LensProfile` 增
     `mask_warp` + `mask_warp_src` 两键且该结构 `deny_unknown_fields`；
     `EditRecipe::lens_profile`（`recipe.rs:380`）与块内两新键都**无**
     `skip_serializing_if`（主审勘误：全结构唯一一个 `skip_serializing_if`
     在 `quarter_turns`，与此无关），所以**每一份** v0.35.0
     写出的 `recipe.json` 都带这两键 ⇒ **v0.34.0 及更早的 exe 具名硬拒
     整份配方**（不是只拒带镜头档案的那些，也不是静默丢帧）。用户可见：
     旧版本打不开新版本存的 develop。反向无损——v0.35.0 读一切旧配方，
     `mask_warp` 空 = 恒等，渲染逐位不变。
  2. **参数形蒙版换帧**（B3，`95c2705`）。LR 把径向/线性放在**镜头校正后**
     的帧、把笔刷放在**校正前**的帧；引擎此前两者同帧读。现在径向/线性走
     `MaskUnwarp`（重采样器的精确逆，非第二个求解器），笔刷走恒等。旧配方
     行为：**几何激活**（distortion/CA 有数据且开）的帧上，带径向或线性
     蒙版的配方渲染改变——落点 0.09–0.30px，未接线时是 8–24px；无镜头档案
     或几何关闭的帧逐位不变。
  3. **笔刷实装**（B6b，`fed4f80`）。Brush 臂从字面 `=> 0.0` 变成真渲染
     （`render::brush_raster` 冲压 dab 流）。旧配方行为：**一切含 LR 笔刷
     蒙版的配方渲染全变**——此前该蒙版等于不存在。两向披露随行为改名
     `BrushCarried` → `BrushRendered`（alpha 是我们量出来的模型，不是
     Adobe 的光栅器）。
  4. **质感换形**（B8-2，`b583e2e`）。负 `texture` 从陷波换成双低通并联的
     高架台阶 + 双曲深度律，绑 45 个 LR 锚点。旧配方行为：**一切
     `texture < 0`（全局与每蒙版）重渲**；`texture ≥ 0` 逐位不变。本分支
     第二次换形（v0.34.0 那次是陷波带限）。
  5. **径向羽化衰减换表**（`0ee386d` + me3 插列 `a841ec3`）。三个被证伪的
     闭式之后落库实测 290×11 表，`Feather = 0` 保留解析硬边。旧配方行为：
     **Feather ≥ 10 的径向全部重渲**（Feather 100 处旧律画的面积是 LR 的
     2.08×）；Feather 0 逐字节同一；Feather ≤ 5 只按旧律自身残差微动。
     me3 插列使 f ∈ (10, 75) 相对 `0ee386d` 版**再变一次**——同一伞下，
     **不新增第八项**。
  6. **AI 蒙版全部重算一次**（B4，`790825e`）。主体后端换成钉 commit 的
     BiRefNet（`e2bf8e44…`）+ 具名 U²-Net 回退，`AI_BACKEND_GENERATION`
     1→2。旧配方行为：v0.34 及更早缓存的 alpha 在首次 develop 时各重算一次
     ——**三个子类型都重算**（sky/object 后端并未换，只是共用一个代号常量；
     用一个字段换三个字段是刻意取舍）。回退过的机器在装上依赖后还会**自己
     再升一次**，并把原因说出来。
  7. **半像素**（C2，`2ae1b11`）。全蒙版族（径向/线性/笔刷/位图/AI）改在
     **像素中心** `(x+0.5)/w` 采样——两张不同底片实测 LR 就是这么采的。
     旧配方行为：**一切蒙版整体上移左移各 0.5px**，与羽化、几何、画幅无关。

  **R30 候选池（本批追加，均未拍板）**：①`crs:Gesture` dabs 转 SAM
  提示点/框（me5 定案：subtype=0 下手势是分割模型的区域提示，引擎 object
  后端现仅用 `ReferencePoint`；属**分割输入**保真，非渲染 gap）；
  ②`crs:MaskBrushTable` 格式侦查 + 读侧纳管**或具名拒收**（me5 附带发现：
  现代 LR 重写 sidecar 时把 14 个文本 `Mask/Paint` 换成压缩 blob，
  44,540→19,261B；现库全是旧写法故今日零影响，但被重写过的文件笔刷无从
  导入，且「是否走 `parse_brush_group` 既有拒收臂」未验证——静默丢笔刷是
  ...
  **⑪（M-C 审计批追加）**eval 报告低 n 行稳健化——n<20 行加标注或按 n
  加权（25% 的行贡献 54% 的分、n=1 单行能抬 1.5pt=2026-08-22 审计
  实证的机制缺陷），及跨跑方差的重复跑协议（verifier 归因唯一手段）。

  **me5 迷你包台账（3 对，2026-08-21，主审第一方判读，无子代理，
  `~/.claude/plans/r29-materials/me5-results.md`）**：**①手势 subtype=0
  删除对照**——`changed_px = 511,020 / 60,217,344`、`max|Δ| = 74 DN`、
  bbox x[4096,5319] y[2896,3895]，12 个 dab 全落在 bbox 内；**内置对照
  干净**（同帧两个 Sky 蒙版 subtype=2 无手势，bbox 外 6000 万像素
  `changed = 0`）；nogesture 侧 Object 组件仍在（Mask/Image×4 +
  ReferencePoint×4 两侧同）⇒ LR 没删组件，是**用剩余输入重算出了不同的
  alpha**。合并 me3-c 的 subtype=1 零贡献（39M 像素 `max_abs_diff = 0` +
  熵编码段逐字节同）⇒ **两个子类型下「渲染期手势合成」都不存在，引擎
  携带不渲染在两边都正确**，零 v0.35.0 义务。**②sq 盒子 Angle=30**——
  a000 对照自证（主轴 −0.073°、矩比 1.1923 对标称 1.2），a030 实测主轴
  **+30.013°**（y-down 顺时针，误差 0.013°），且角点代数的**轴比与面积
  双预言同时命中**（预测 5.787 vs 实测矩比 5.8037；预测 0.342 vs 实测
  权重比 0.338）＝R26 角点代数在**第三个几何**（近正圆盒子）上像素级
  验证，me3-b 告警 #6 关闭。**③附带**：`MaskBrushTable` 前向兼容登记
  （见 R30 候选②）。两实验均 CONFIRMED 级。

  **F2 census 计数待溯源（me3-c C-2，本批只标注不改数）**：`recipe.rs`
  的「83 of 218 components」与 `xmp.rs` 的「218 Mask/Image」「382/382
  Paints」「300/300」「135 of 218」在今日 `D:\Photography\**\*.xmp`
  （177 份 sidecar）上**全部不复现**——实测 105 Mask/Image / 398 Mask/Paint
  / 1081 Mask/\* / 40 gesture 块；R28 MANIFEST §5-3 独立报同一现象。判断
  ＝**变的极可能是 census 走的语料根**（F2 那批的记录里没写根路径），不是
  图库。处置＝**溯源前一个数字都不改**，只在 `xmp.rs` 的
  `AI_MASK_PROVENANCE_KEYS`（全族锚点）与 `recipe.rs` 两处 census 段各挂
  一条「计数待溯源、不变式不受影响」的注记；代码没有任何一处依赖计数
  （allowlist 与拒收依赖的是词汇表与不变式，两次 census 对这些一致）。

- **R29 me3 径向插列批已落地（2026-08-21，未发版，`a841ec3`）：
  `RADIAL_FALLOFF` 290×8 → 290×11，插入实测 f=15/35/65 三列——列间从
  外推变实测**。根因＝径向 LUT 落库批（`0ee386d`）自己登记的「列间未
  测量」残余：me3 包 01 的三张受控导出把它变成实测——八列表在 f=15 读
  rms(α) **0.0209**（max 0.0799）、f=35 读 **0.0212**（max 0.0490），换算
  成 α=0.5 等值线在实测帧主轴上偏 **+14.8px / +24.9px**（`a_07` §4）；
  f=65 本来只有 0.0004（0.2px），因导出已存在顺带插入。**换插值函数族
  全劣于插实测列**（pchip-log-f 0.0092 / cubic-log-f 0.0093 / akima 0.0105 /
  pchip 0.0113 vs 现行线性 0.0141，而插列后同三档 = **0.0000**，`a_08`）
  ＝**家族不动、只加列**，与落库批「常量表 + 线性×线性」架构零冲突；
  全梯 held-out 均值 0.0268 → **0.0140**（`a_07` §2）。**表数值零手打**：
  `a_09_table.py` 的产物 `RADIAL_FALLOFF_11col.rs.txt`（27,725B，sha256
  `cd993fc7…`）整体 splice 进源码，290 行字面量与 f 阶梯行**逐字符**
  相同、f32 位型全等（复核脚本亲跑），**旧八列逐位保留**
  （max|Δ| = 0.000000 / 2320 项）；调理代价随实测更新（PAV ≤0.0073
  ＝新三列 0.0073/0.0072/0.0060，running-min 新列 0.000000）。
  **⚠渲染行为变更：不加新硬变更项，并入 v0.35.0 置顶清单第⑤项径向 LUT
  的同一伞**——f∈(10,75) 的旧配方相对 `0ee386d` 版**再变一次**（列上按
  该列自己的 held-out 误差、列间按新邻居重插）；f≤10 / f=25 / f=50 /
  f≥75 这些列本身逐位不动。schema 零改动。**注释改真三条**：①列数 /
  阶梯 / 来源（八列 B7-2 §3.1 + 三列 me3 `a_05`，带 held-out 与像素位移
  数字）；②「ASPECT INVARIANCE is unsampled」→ **已采样一次**（held-out
  长宽比 1.2 上 rms(α) 0.0009、max|dev| 0.0031、α=0.5 位移 +0.04px，
  最佳单一径向缩放 k=1.00076 ＝无任何像素锚定成分；作用域保留＝只
  f=50 一档、只一对长宽比、仍居中——me3-b H1/A1-A2）；③d_out：
  `1.43±0.015` 与 `1.4335` 撤销 → **√2 采纳**（四件**形状无关**仪器互证
  ＝扇区块终点 1.41480±0.00046 / 符号普查支撑止于 ≤1.4200 / 全变暗块界
  1.41367 / 环均值末显著带 1.4142，正向检验把 1.43 与 1.4335 的预测端点
  推到实测之外；残余系统差 ±0.001 如实声明。表不显式落 d_out，故这条
  **零像素差**——me3-a §0-Q2）。**顺带勘误两处**（亲算，非 me3 产物）：
  ①复现测试注释「Twelve of these 136 … the rest sit within 0.0016」——136
  个里实为 **9** 个超 1.0（扩到 11 列后 187 个里 **11** 个），「其余」实测
  在 **0.0013** 内（旧 8 列表算出来同样是 0.0013）；②`approx_constant`
  三个撞数学常数的值里「0.3659 near LOG10_2」实为 **0.3010**
  （ρ=0.7975, f=90；摘掉 `allow` 让 clippy 亲报得到），另两处 0.4342
  （ρ=0.7425, f=75）/0.4343（ρ=0.8225, f=50）属实，三个全在旧列里。
  **新增 drift 门** `the_radial_falloff_table_is_the_generated_artefact`：
  对整表 3190 个 f32 的位型与 f 阶梯各取 FNV-1a-64，钉的是生成物本身。
  门=clippy 0×2 + **816(9i)/14/132/2+2** 双配置（+1 净集差＝该 drift 门；
  两个既有表测试的 `PUBLISHED` 由 8 列扩到 11 列，期望值取 me3 `a_05`
  稠密表，**其八列与既有断言逐位相同＝转录自校验**）+ check_docs 20P0F
  + fonts 803/803 + **LR 探针 16/16**；变异 3 组、全部 sha256 字节还原
  亲证：①新列 +0.002（刻意压在 0.0025 容差以下）→ **只有 drift 门红**；
  ②阶梯 15→20 → **只有 drift 门红**（实测揪出的洞：既有复现测试是
  **读**阶梯而非**钉**阶梯，标错列名它看不见）；③旧列 −0.01 → 复现 /
  结构不变量 / drift **三红**。登记不做：f≤10 端列间缝仍未测（drop f=5
  从 1/10 预测 rms 0.0253，但该区过渡窄 W≤0.17，同样 α 误差的像素位移
  远小于 f=15/35——me3-a §4）；me3-b 的 H2（Roundness 负半支 + 扇区
  作用域两句）与 H4（V2_PLAN §7 item 1 折叠分支归因）属另批文档改真，
  不在本批范围。
...
- **R29 收口批 C2 已落地（2026-08-21，未发版，`2ae1b11`）：全蒙版族改采样
  ...
  **既有标定不需要补偿平移，逐条论证**：①径向 LUT 的 ρ 是对**实测拟合**
  ...
  全 290×8 表零改动（本批当时的形状；me3 插列批后为 290×11，该论证不受
  影响）且 `texture_negative_hits_the_forty_five_lightroom_anchors`
  ...
  **登记不做 / 未验**：①~~`AUTOSHOP_MB_FIXTURES` 的 7 份 M-B 取证集**本机
  遍寻不获**（`P34.xmp`/`P31.xmp` 全盘无命中），故该门本批
  **未跑**，非绿非红~~ **← 主审复核推翻**：取证集在
...
- **R29 传输重试批已落地（2026-08-21，未发版，`74ffa25`）：524/529 纳入
  ...
  （529=Anthropic 过载确定未计费重试零风险；524=中转网关超时计费未知，
  接受最坏「该张重复计费一次约几美分」换整跑不废稿——attempt-3 因
  4×524+1×529 触发冻结判据废稿即本批消灭的故障模式）；**②W_EMB 第二
  标定臂随 v0.35.0 发版重基线同窗跑**（发版义务清单+1，费用集中一次
  审批）。实现＝`worth_one_repeat` 双嘴单漏斗（B1a 流事件+新
  `TRANSIENT_STATUSES` 状态表，各携自己的计费句——两类唯一差异处）；
  **只认状态码不认 body 文案**（429/503 带「overloaded」文案照旧终态，
  测试钉）；**一次重试帽跨类共享**（忠于拍板定价「至多一笔重复计费/张」；
  主审简报「每类一次」系过度展开、代理纠偏采纳=本轮第三例）；
  `spend/settle_repeat` 单点管帽/停/披露/计数，六出口全包裹（`is_ok`
  承重）；eval 报告快照差额披露行（净跑静默、救回/仍回退分列、524
  重复计费句在场）。**同根并修**：B1a 登记的 assemble_sse error 键
  预检 wart（同类=漏斗认不出上游具名的瞬态）改 type 先行+非扩宽钉。
  刻意不纳管带理由登记：generative 图像编辑（重复计费量级远超拍板
  定价）+免费 /models GET。门=clippy0×2+**794(9i)/14/131/2+2** 双配置
  （+7 集差：代理 6+主审补钉 1）+check_docs 20P0F+fonts 803/803；
  变异=代理 7 组（M4 初存活揪出 is_ok 死代码→重构承重）+主审亲手
  2 组（去 529→双红；**计费句对调→67 测试全绿存活=真缺口**→补
  `each_transient_status_discloses_its_own_billing_truth` 钉，对调红
  双向亲证）。登记=未对真中转实弹验证（回环桩台，计费语义引拍板档）。

- **R29 径向 LUT 批已落地（2026-08-21，未发版，`0ee386d`）：径向羽化衰减
  从第三个被证伪的闭式（`ramp(1−f, 1+f/2)`）换成实测本身——
  `radial_falloff()` 290×8 查表，R29 渲染修复三连收官**。表=八列实测
  羽化档（1/5/10/25/50/75/90/100）、行=测量自身 bin 中心（§3.1 逐位
  复现处零重采样）；f=0 解析硬边（实测列 0.0084 宽系 JPEG 模糊地板
  非 LR 边缘）与旧退化 ramp 字节同构；双调理具名带代价（ρ 向 PAV
  ≤0.0061、f 向 running-min ≤0.000061 且止于椭圆=圈外次序真实反转
  不强扭）；插值线性×线性有据（W/f 散布 1.97× vs √f 7.5×、held-out
  对曲线对手符号翻转）；**无 d_out 常数**（支撑烘进列尾，1.43±0.015
  与 √2 未决保持登记）；NaN 羽化/NaN d 双守卫（代理自查修掉初稿
  NaN 洞并钉测试）。**约束达成=LUT 八档全面反超旧律**（rms ≤0.0009
  vs 0.009-0.156、α≥0.5 面积比八档全 1.000 对生剖面测非循环自证）；
  消费者普查=Linear 无羽化字段、ramp 余四调用者全非径向（主审亲验）；
  帧适配/旋向/求值点/极性钉全保持。**⚠渲染行为硬变更：feather≥10
  的一切径向重渲**（F100 时旧 α≥0.5 面积 2.08×、等值线偏 239px），
  f=0 字节不变。门=clippy0×2（表内三值撞数学常数 scoped allow 带 why）
  +**787(9i)/14/131/2+2** 双配置（+3 净集差：表复现 tol 0.0025 带
  防虚化地板/逐档胜旧律/硬边+连续性/结构不变量四新−1 旧端点钉删）+
  check_docs 20P0F+fonts 803/803；变异=代理 9 组（M5 初存活揪出支撑
  断言全落在过界早返回的真洞→补表内尾行）+主审亲手 2 组（**半格
  错位→红 0.0026 恰超 0.0025 容差=紧到极限**；f 插值方向反转→红
  0.0315）全字节还原。登记=列间未测量（最宽间隙 ~0.05 外推）/长宽比
  不变性未采样/f=1 远尾未决/V2_PLAN:131 Linear smoothstep 草图行
  drift（先在陈述、非本批产物）。**至此 v0.35.0 发版置顶义务累计：
  LensProfile schema 硬断裂+参数形帧变更（B3）、笔刷实装（fed4f80）、
  质感换形（b583e2e）、径向 LUT（本批）——发版说明四项全置顶。**
  **← me3 收紧注记（2026-08-21，详见「当前状态」首条）**：本批登记的
  三条残余中，「列间未测量（最宽间隙 ~0.05 外推）」已由 me3 插列批
  关闭（290×8→290×11，实测 f=15/35/65），「长宽比不变性未采样」带界
  关闭（held-out 长宽比 1.2 上 rms 0.0009，仍只 f=50 一档），「1.43±0.015
  与 √2 未决」改判 **√2 采纳**（1.43 与 1.4335 被判定性排除）；f∈(10,75)
  的渲染相对本批版本**再变一次**，但仍归 v0.35.0 置顶清单第⑤项同一伞，
  **不新增硬变更项**。

- **R29 质感换形批已落地（2026-08-21，未发版，`b583e2e`）：texture 负半支
  从被证伪的 R28 陷波换成 B8-2 实测高架台阶——双低通并联×双曲深度律**。
  实现＝`texture_negative_pass`：A₁=0.172443/σ₁=0.31235% 短边 +
  A₂=0.304888/σ₂=0.02822% 短边（B8 被锐化藏掉的细臂=64% 深度）、
  w(t)=t(1+d)/(1+dt) d=0.558583（线性 strength=-amount 废除）；σ 绑
  **渲染栅格**短边（本引擎全幅显影+末步 resize 恰好递入）。**核选择由
  45 锚点网格仲裁**（±0.02 预算=b8-analysis-2 §6-3）：整数/分数 box³
  双双落榜（0.044/0.037），落地=细臂真高斯 FIR+粗臂经高斯半群从细平面
  按残差 σ′ 长出（仍两 f32 平面、峰值预算不动），最差锚点 0.0037；
  网格紧度断言 0.006（半群丢失=0.0092 必红）。**预览钳位（拍板三）**：
  σ 亚像素臂跳过+如实披露（GUI 预览负质感诚实偏弱、导出精确；钳位
  非行为悬崖=σ0.24 的采样核本就传 0.9993，已注明）；短边 <228px 整个
  负半支拒渲（防单臂错配）。AI 缓存免换键（读码定案：分割器吃
  decode 帧非显影像素，segment.rs stage_source_frame→decode_any_turned；
  render-fed 路径全默认配方 texture=0）。gamma 域承重已由构造满足
  （develop 缓冲 sRGB 编码先行，:326/:1417 引证）。**⚠渲染行为硬变更：
  一切负 texture（全局+蒙版）重渲**（本分支第二次换形：v0.34.0 陷波
  为零真值时代自造、受控阶梯证伪其形）；正半支逐字未动；位级孪生
  纪律保持且断言非虚化。门=clippy0×2+**784(9i)/14/131/2+2** 双配置
  （+3 净集差：45 锚点网格/形状史/σ+钳位/位级孪生四新−1 替换）+
  check_docs 20P0F+fonts 803/803；变异=代理 11 组（含两条过程披露：
  变异中途超时污染自查修复+M8 初绿收紧紧度断言）+主审亲手 2 组
  （细臂振幅回退 B8 污染值 0.0928→锚点红 0.0201；粗臂回归级联带形→
  形状+锚点双红）全字节还原。登记=非 LTI 保边不建模（系综对齐如实
  注明）/B8-2 §7-1 σ 底片分辨率二义不闸本架构。实现队列余=③径向九列
  LUT（f→0 硬边退化约束）。

- **R29 笔刷渲染实装批已落地（2026-08-21，未发版，`fed4f80`）：
  `MaskGeometry::Brush` 从「只携带不渲染」（R27 Batch-4 起字面 `0.0`）
  变成真渲染——B6 实测模型全量当测试锚点钉死**。实现＝
  `render::brush_raster` 在显影期把 dab 流冲压成 8-bit 灰 alpha
  （行并行+自平衡工作预算：2048 长边/24M 求值帽/32px 地板、峰值
  ~37MB 居既有蒙版栅格包络内、按（流哈希+帧尺寸）记忆化），
  `mask_weight` 与 Bitmap/AiMask 同法取样；核形 (1−ρ^m)^n 双三次、
  κ=0.1284 odds 律 D(1)=1 精确、screen 累积、笔画 value=浓度 pre-screen；
  Aggregate 的 MaskValue 恒不作强度（subtract 对）、组级 MaskInverted
  被渲染（1−α 且仅在有 alpha 处＝空组不得整幅化）；dab 坐标零 warp
  （镜前帧，B3 谓词不含 Brush 钉住）。**零 schema 改动**（栅格=渲染期
  工件）。**⚠渲染行为硬变更：一切含笔刷蒙版的配方从此渲染不同**
  （原静默不画→真画出来），披露双向改名 `BrushCarried→BrushRendered`
  （两 enum 零 serde 亲验=非兼容性事件）措辞改「由本机实测模型渲染——
  非 Adobe 原栅格」，README/ARCHITECTURE/GUI 徽章与 i18n 全链同步；
  **v0.35.0 发版说明置顶义务+1**（与 LensProfile 硬断裂、径向/参数形
  帧变更并列）。门=clippy0×2+**781(9i)/14/131/2+2** 双配置（+8 具名
  笔刷测试：九档核表复现/流量律/screen 判别/闭式冲压/尺寸策略三界/
  ...
  ⑤全尺寸导出栅格 2048（4.6× 粗于 9504 帧、预览 1:1）。实现队列余
  ②质感换形（B8-2 §6 参数包+预览钳位拍板在案）③径向九列 LUT
  （f→0 硬边退化约束）。

- **R29 追加分析批 B8-2/B7-2 已裁决（2026-08-21，纯分析零码变更，docs 批
  落账）：追加素材（me2 包 15 对，用户当日交付含半尺寸重导）消化完毕，
  三个渲染修复批的素材闸全开**。报告+主审裁决（亲跑复现）：
  `~/.claude/plans/r29-materials/{b8,b7}-analysis-2.md` 各末节。要点——
  **B8-2（质感干净基座）**：B8 数值被 Sharpness=150 混淆重度污染
  ——形状（单调高架台阶、无陷波）与中频 −13 dB 过度平滑**保持**，但
  平台深度被低估 66%（真值 H∞=0.549@−100）、4px 实保 0.57 非 0.72、
  「低频 +1.8% 抬升」纯系锐化产物撤销、细端欠平滑恶化至 +5.26 dB@3px、
  函数族错判 2.3×→**8.2×**；五点阶梯证伪单一幂律→**双曲深度律
  w=t(1+d)/(1+dt), d=0.5586**（最大偏差 0.008）；非 LTI 确认为 Texture
  自有（干净跨度 +0.52 vs 引擎 LTI 对照 ≤0.009）；**σ 归一判定（半尺寸
  重导对）=传递按短边归一频率不变（16× 分离+留出验证）→σ 绑渲染栅格，
  本引擎「全幅显影+末步 resize」架构（main.rs:891-894）恰好满足=只改
  常数与函数族不改归一**；域选择承重（gamma/linear 在 4px 差 0.04，
  必须 gamma 域实现）；参数包+45 锚点在 §6；预览 σ₂ 亚像素=设计决策
  待拍板。**B7-2（径向补样+交叉）**：**B7 的 d_out=1.4335±0.002 精度
  系 JPEG 8×8 块外溢假象**（带内零均值+扇区按 8/a vs 8/b 分开+四侧
  mod-8 对齐 12 检验 p≤1e-23）→修正 **1.43±0.015**、√2 回到
  UNRESOLVED（16-bit TIFF 可收紧，用户已跳过该可选步骤）；f∈(0,25)
  连续张开无跳变；a≈1.9/f 候选律证伪（f=1 偏 58×）→九列 LUT 定稿
  （f=0/1/5/10/25/50/75/90/100）；**新实现约束=现行 ramp 在 f≤5 本来
  就对（rms 0.009），LUT 必须 f→0 精确退化硬边**；α(0)=1 扩至八档；
  Roundness=100 no-op 扩至 Feather=50（B7 最大漏洞闭，|Δα|≤0.006；
  探针实为不同底片——主审简报前提错误由代理纠出并免标定重设计，
  本轮第二例）；两导出会话逐像素恒等（max|Δ|=0）。**实现队列（三批
  全解锁，均为渲染行为硬变更+发版置顶义务）**：①笔刷接线（B6 §8）
  ②质感换形（B8-2 §6）③径向 LUT（B7-2 §6）；残余登记随批披露=
  长宽比不变性/负 Roundness/第二几何/f=1 远尾/预览钳位拍板/两底片 2%
  乘性差溯源。本批文档真化=V2_PLAN §7 items 1/6/11 三处修订层、
  render.rs 质感块重写+径向块+圆度块补层。

- **R29 分析批 B6/B7/B8 已裁决（2026-08-20，纯分析零码变更，docs 批落账）：
  M-E 素材三线拟合收官，三条现役设计被实测证伪、两条历史台账修正**。
  三份报告+主审裁决（含亲跑复现记录）：`~/.claude/plans/r29-materials/
  {b6,b7,b8}-analysis.md` 各自末节。裁决要点——
  **B8（texture 负半支）**：R28 B5 陷波形状证伪＝LR 实测为单调高架台阶
  （半深拐点周期 ~49 px、平台 H∞=0.729 @−100、4 px 细节 LR 砍 28% vs
  引擎 0.08%），深度 |t|^0.74 亚线性（引擎严格线性）、算子非 LTI（保边）；
  引擎带形自由重拟合仍 2.3× 差于双低通并联混合＝函数族错误；实现替换被
  Sharpness=0 阶梯+双分辨率导出两素材阻塞（夹具模板残留 Sharpness=150
  为头号混淆项）。
  **B7（径向羽化+双探针）**：d_out=1.4335±0.002 与 feather 无关（渲染
  衰减端点 1+f/2 证伪；batch-8「饱和 1.41」与 batch-10「d_in=0.79−0.94f」
  均为单 smoothstep 硬跨真剖面的投影）；α(0)=1 四档全部＝无内膝，无两参
  闭式达底噪→落库形态=实测 LUT；现行 ramp rms(α) 0.093–0.156、f=100 时
  α≥0.5 面积 2.08×＝**当前最大已量化活渲染 gap**；Angle=60 实测 59.995°
  ＝|Angle|>45 解码首获像素验证（V2 §7-1 该句关闭）；Roundness=+100 实测
  精确 no-op（限 +100/Feather=0/单几何，V2 §7-11 升级）；G4 半像素采样
  约定（引擎 x/w vs LR (x+0.5)/w，0.5 px）跨蒙版族登记待单独拍板；G5 帧
  尺度又一 profile-OFF 样本 0.99874，维持 1.0 拍板不动。
  **B6（笔刷核三合一）**：核形闭式存在＝(1−ρ^m)^n 两条三次曲线 8 数
  （held-out 0.0109 优于表插值 0.0180，batch-10「无闭式」被接替——第二
  参数该花在 ρ 指数上）；κ=0.1284±2.24% 普适（3× 半径+双硬度端；与
  batch-10 差 5.3%≈1.6σ 如实并报，odds 律每档 κ ~11% 漂移登记）；screen
  20 拖复确认（1.8×/3.4×）、D(1)=1 零自由参数；**Size=α≈0.30 直径**
  （免插值 c\*=0.2974，spread 0.20%；batch-8「α0.5 直径 266.1px」证伪＝
  `id` 律+名义帧遗产）、存储 Radius=导出量（预测 1.4 px rms）＝(Radius,
  CenterWeight) 单自由度；Feather→CW 九点两闭式（Padé[2/2] rms 2.2e-5）。
  **两项 AskUserQuestion 拍板同日落档**（`r29-rulings-2026-08-20.md`）：
  ①eval 传输 524/529 都纳管重试一次（带披露+计数，B1a 同根扩类，下个
  代码批落）；②W_EMB 标定臂随 v0.35.0 发版重基线同窗跑（发版义务+1）。
  **候选池刷新**：笔刷接线批（B6 §8 清单，素材零阻塞+零 schema，
  MaskWarpSource diag/GUI 面合流）＝下个实现批首选；径向 LUT 与 texture
  换形＝素材阻塞（追加素材统一指令包待交付：Sharpness=0 三档、双分辨率、
  f=10/90+f=1..10 微阶梯、Roundness×Feather 交叉、16-bit TIFF 可选）；
  三者落库时均为渲染行为硬变更＝发版说明置顶义务已挂。本批文档真化=
  V2_PLAN §7 items 1/3/6/11/13 五处、render.rs 三注释块+测试注释、
  recipe.rs 字段文档、fixtures MANIFEST 行号勘误。

- **R29 Batch-3 已落地（2026-08-20，未发版，`95c2705`）：镜头档案蒙版帧根治
  ——三轮「1.032 之谜」端到端关闭**。当日三段证据链（全部主审裁决在案，
  细账 `~/.claude/plans/r29-materials/me-results.md` D 实验节+lcp-recon 档案）：
  ①侦察段（Opus 只读+scratchpad 原型）：`.lcp` 模型零自由参数打到测量噪声
  地板（11 dab rms 4.63px @地板 4.19px；138 像素补丁 2.11px；held-out flow
  梯 2.80px 反超批 10 自家拟合 3.80px）；**引擎早已从索尼 0x7037 机内节点
  算出同一张图**（`profile_fill_scale` 与 Adobe ScaleFactor 相符 1.3e-4）；
  角部 1.032＝该帧场的角部值被误当常数。②D 实验（用户 2 分钟两导出，
  30 NCC 点）：105mm 像素被档案场精确扭 +87.5px（rms 2.69）而批 8 实测
  径向蒙版≈恒等（差 88.7px）→ **LR 按蒙版类型分帧：笔刷 dab 存镜前帧、
  径向/线性参数形存镜后帧**。③实现代理阻断发现（主审一手确认
  render.rs:379 vs :404）：本引擎先算蒙版后扭几何 → 笔刷臂天然正确、
  **径向/线性臂活错最多 186px(24mm)/88px(105mm)**——与我方最初分臂指令
  相反，代理拒绝执行错误指令获采纳。**接线（用户拍板本批落地）**：
  `render::MaskFrame` 单值同决「几何段跑不跑/蒙版链适不适配」（构造条件
  与 :403 门同源＝门与映射不可漂）；`MaskUnwarp` LUT **调用**
  `lens_ungeom_norm` 而非第二实现（我方重采样精确逆，手动畸变量零残余
  覆盖；`.lcp` 派生图留给笔刷帧+来源标签，职责具名不重叠）；
  `is_lr_post_correction_geometry` 单谓词钉分臂（范围蒙版按值选择帧不变、
  AI/位图引擎自产，各带理由）；GUI 两处取样参考显式 `AsRendered`；
  验收实测＝接线后落点误差 0.09–0.30px vs 未接线漂 8.4–23.8px。
  **⚠深思熟虑的渲染行为变更（随 v0.35.0 硬断裂）**：镜头几何激活帧上的
  已导入径向/线性蒙版改渲在存储坐标（向 LR 对齐最多 186px）；几何不激活
  帧字节不变（测试钉死）。**基座同批**：新 `src/lcp.rs` 1090 行（双拼写
  XML/f64 牛顿逆带折叠检测+收敛回代验证/ScaleFactor 只读不派生——恒等式
  在五个无穷远节点 2e-4 成立、近摄节点 2e-3 反例断言为地板/鱼眼具名拒绝/
  ProgramData+AppData 发现缓存+AUTOSHOP_LCP_DIR 逃生阀/**色差子模型泄漏修
  ＝3576 档中 707 档会把色差通道 ScaleFactor 捐给整条目搜索**）；
  `LensProfile` +`mask_warp`+`mask_warp_src`（7 态类型化+en()+clamp 强制
  标签/数据不变式）＝**硬前向断裂：v0.34 exe 拒读含镜头档案的 v0.35
  recipe**（复刻旧反序列化器断言）；`lensmeta::read` 双源解算（A=机内
  节点/B=.lcp）具名拒绝；`xmp::lens_profile_enabled` 双拼写读。门（主审
  亲跑）：clippy0×2、**773(9i)/14/131/2+2 双配置**（749→+24 集差逐文件
  归属）、check_docs 0 FAIL、字体 803/803；变异＝代理 13 组（M1/M12 初绿
  各揪出真夹具洞：色差泄漏/平坦激活档案）+主审亲手 2 组（谓词删 Radial→
  落点红；牛顿砍 1 步→往返红）全字节还原。**登记**：焦距插值线性未验证
  （34mm 判别力仅 0.5px）；sff 回落只在 sff=1 演练过；MaskWarpSource 的
  diag/GUI 面留待笔刷消费批；develop_preview 缺省改配方派生＝新脚枪已文档
  （未来不扭几何的调用方须显式 AsRendered）。
...
- **R28 v0.34.0 已发布（2026-08-20，全五批+发版链；托管夜间轮）** —— 批次范围
  ...
  （.lcp ✅B3/羽化调和 ✅B7 裁决（LUT 形态，实现待补样）/Feather→CenterWeight
  ✅B6 九点闭式/κ 普适性 ✅B6/export-at-size ✅B2/BiRefNet（仍待目检闸）/
  `|Angle|>45°` 径向角度折叠未验证 ✅B7 像素验证 59.995°（xmp.rs
  `canonical_lr_angle`；竖幅/裁剪帧本身 R27 Batch-3 已关，2026-08-20 深检勘误）/
  诊断 sink ✅B1/XMP Scope 全域化余项 ✅B1/texture 负半支 LR 地面真值 ✅B8
  裁决（形状证伪，实现待补样）——✅标注均 2026-08-20 后补，指向各批台账）。
...
- **R28 Batch-5：语义/口径批（2026-08-20，~~未发版~~ →已随 v0.34.0 发布）** —— 5a **texture=−100 端点**
  ...
  **未测量登记**：本仓无 LR 负 texture 地面真值（没有 −100 受控导出可拟合），
  ...
  XMP 仍写原始滑杆值，LR 用自己的模型重渲）。**← 该登记 2026-08-20 由 R29 B8
  关闭且裁决为形状证伪（LR 实测=单调高架台阶非陷波，细节端反而砍更多；端点测
  0.963/0.294 仍真，但那是相对旧分支的改进量、不是 LR 行为，且非分辨率不变
  ——见上方 R29 分析批条目与 `b8-analysis.md`）。**端点测钉的是**形状**：800px 帧、
...
- **AI 蒙版重算 + SigLIP 2 风格嵌入（2026-08-19，R27 Batch-5，~~未发版~~ →已随 v0.33.0 发布）** ——
  ...
  **T2 · `Mask/Image` 臂落地（V2_PLAN §7-14，L-08 Arm C）——剩余拒收的主项**。
  ...
  ⚠**这段里的每个计数都待溯源**（me3-c C-2，2026-08-21）：今日语料上
  一个都不复现（见「当前状态」文档 sweep 条的 census 段）。**不变式
  ——21 个属性名、三个 `MaskSubType` 值、无栅格无几何——两次 census 一致**，
  本段的结论不受影响；数字在拿回当年的语料根之前一个都不改。
  ...
  **M-B 夹具前后实测（同一条测试、同一份夹具，HEAD `b39044f` worktree 对照）**：
  `P04` 13 条 correction **8 → 13** 导入（拒收 5 → 0）、`P05`
  **3 → 5**（拒 2 → 0）、`P10` **3 → 5**（拒 2 → 0），其余四份本来就 4/4、
...
- **画笔蒙版结构落地 + 分割许可证根修（2026-08-19，R27 Batch-4，~~未发版~~ →已随 v0.33.0 发布）** ——
  ...
  **同时**带 `Mask/Image`，所以仍拒——33/42 前后一致。真正的改善在有纯笔刷素材的
  目录上：`P12` **8/11 → 10/11**（Mask 1 与 Mask 7 进来了，各带
...
- **台账勘误 + 登记转正（2026-08-19，R27 Batch-1a，~~未发版~~ →已随 v0.33.0 发布）** —— 不是一次发布，
  ...
  **① 素材台账（M-B / 六合一实验）**：`M-B` 行的「Defringe 非零仍欠」与
  ...
  Defringe 非零 `P17.xmp: DefringePurpleAmount="10"` / 径向旋 +30°
  `P19.xmp: Angle="28.229232"` / 蒙版 Hue +50 `P14.xmp:
  LocalHue="0.277778"` / 蒙版 Sharpness `P14.xmp: LocalSharpness="0.803738"` /
  Subtract 复制 `P18.xmp: MaskBlendMode="1"` / Auto CA `P16.xmp:
  AutoLateralCA="1"`；另有两份附赠 `P15.xmp: Sharpness="150"`、
  `P13.xmp: LocalExposure2012="1"`。
  ...
  **⑧ V2_PLAN / M1_PLAN 同步**：§7-2 正端点关闭（`P13`）、§7-3 折入三组
...
- **裁剪几何 + 画幅根修（2026-08-19，R27 Batch-3，~~未发版~~ →已随 v0.33.0 发布）** —— 把 R27 的三份
  ...
  **④ `CropAngle` 精度 `{:.1}`→`{:.6}`（P3 §6.4）**。导入 `P08` 再存回原本
  ...
  **⑤ 蒙版帧＝未裁剪的源帧（P5 §1，HIGH）**。`probe4/PROBE4-FINAL.md` §4 开头
  ...
  `DefaultCropSize`：`P32_16.9.JPG` 上按导出尺寸解会把五个径向位移
  ...
  **⑨ 解析器加固（P5 §8）**：JPEG 的 XMP 走 APP1，**用户图库里 13 份导出**
  （含 P3 七张之一的 `P30_1.jpg`）超过 64 KB 段限、拆进 GUID 键控的
  ...
  **按幅值**成组写的（211 份导出里 `LuminanceSmoothing>0` 的 4 份全带 Detail、
  ...
  非零**发射，于是 `P33.JPG` 那种形状（Smoothing=50 / Detail=50 /
...
- **v0.31.2（2026-08-18，证据修复批 —— 用户自有图库只读取证再证伪两条既定
  ...
  「LR 渲染器读哪个属性」两种读法下都成立。实测 `P34` 对真 LR 导出
...
- **v0.31.0 RELEASED（2026-08-18，第二十五轮=收尾闭合轮：Lightroom 差距全量
  ...
  **不碰** `schema_loses`（那才是决定整体回退的谓词，也正是当年判「不值」的理由），
  ...
  ✅ 素材 `P16.xmp: AutoLateralCA="1"` 已在手**；底部变体条物理合并搁置；
...
- **v0.30.0 RELEASED（2026-08-17，第二十四轮=17 条反馈三轮清账收官之 R24：数
  ...
  **17 条反馈全部条目至此代码面收官**。本轮闭环：**#8**（版本命名+出处：
  ...
  `P17.xmp: DefringePurpleAmount="10"`，见「台账勘误（2026-08-19，
  ...
  **后续登记**：底部变体条物理合并（共享属主已铺=删除+重钉小活，待
  ...
  ~~B2-B5 待 Defringe~~ **← Defringe 素材 2026-08-18 已交（`P17.xmp`），
...
- **v0.26.0 RELEASED（2026-08-12，第十八轮=在册清零·分区门量纲化+锚点还债）**
  ...
  P21×reimagine-3 **E=0.0050**（份额比 1.12，天花板下方 3.0×），
...
- **R17（2026-08-12，第十七轮=反推灰纱根修·证据识别自警；feat `50ad2ae`，
  随 v0.26.0 发布）** —
  用户三报同对反推异常（P20 × reimagine-2）：v0.25.0 复合解渲染**灰闷**
  ...
  **判别量三代实测淘汰**：成员 Jaccard（误伤良性嵌套）→ CDF 污染度（haze
  ...
  误差；公式与生产 tone_map 逐步一致；P20=0.021、haze=0.126 均回退，
...
- **自动镜头校正（A 档立项落地，2026-08-03，已提交未推送）**——三批之三。
  ...
  暗角 / 0x7035 CA / 0x7037 畸变，各 16 节点样条，真机 P26 实测齐
...
- **相机基调批次：打开 RAW 不再暗 + 修饰不再切换像素源（2026-08-02，用户
  ...
  `probe_real_raw_base_look`（AUTOSHOP_PROBE_RAW 门控：P27.ARW median
...
- **反馈批次 #2-B：语义分区反推（2026-07-09/10 夜，随 v0.8.0 发布）**
  ...
  真机对（P21 × reimagine-5）驱动迭代 4 轮渲染目视：
...
- **反馈批次 #2-A：反推统计加固（2026-07-09 深夜，`7471d35`，本地未推送）**
  ——用户 v0.7.0 真机复测报"效果还是不好"（P21 × reimagine-5：全图刷
...
- **反推配方修复（2026-07-07，用户真机报障"反推 XMP 之后很奇怪"）**：
  ...
  用户的 P21 真对上逐位复现并分阶段渲染定位，三个根因全部根治
...
- 待用户真机验收（v0.3.0 起累计）：曲线拖拽/吸管/图章/拉直/范围蒙版
  ...
  **v0.5.2 新增待验**：窄窗口下顶栏折行观感；蒙版手柄命中半径（12px）
  ...
  （引擎路径已在用户 P21 真对上复现→修复→复验，GUI 点击链路同函数）。
  **变体条重构新增待验（随 v0.6.0 发布）**：① 生成出片→底部出现「AI 生成」变体
  ...
  ① 新 build 里对 P21 × reimagine-5 重跑反推（Settings「反推」区开关
...
## 当前状态历史（2026-08-24 归档批次：D2 轮 7a/6f/6e/6d/4/3/判别收官 + W6 素材 + 🔒v1.0.0 拍板 + D1 + W2 + R30⑦ —— 逐字迁自主文件，勿改写）
...
- **D2 轮 7a 墙面两对孪生构建批落地=四张待用户导出（2026-08-23；报告
  ...
  几何 (0.18|0.50|0.82)×(0.20|0.50|0.80)、r(0.055,0.045)）+pair2
  `wallr_c/d`=R1 (0.30,0.32) r(0.16,0.11)/R2 (0.70,0.63) r(0.30,0.20)
  与轮 1 d2 逐位同；源=me7_h0000（HasCrop=False ⇒ 导出帧=存储帧
  9504×6336，R2 右缘恰落帧边=历史出幅警示消失）。主审亲核：四 ARW=源
  SHA FE5745…、四 XMP 哈希与报告逐一同、孪生差恰 1 字节 @4906
  （'1'→'0'=LensProfileEnable）、蒙版外前后缀字节同源、9/9/2/2 蒙版
  +SyncID 18/4 唯一、LocalExposure2012=-1（渲染探针 11/11 台阶 ≥1.4257
  ≫0.18 闸、内地板 ≥18.4/255）；**预注册表全部亲算重现**：44 项点律
  （四图心约定）|差| ≤0.001px、field 族三基线逐位=锁定墙面表（该族定
  义=蒙版中心点 (T+R)/2 亲证）。判决批 7b 待用户导出：成对点律 ≤1px
  全格（G2/G8 纯 y ±29px、G3/G6/G9 右侧、R1/R2 尺寸锚）+非成对四族膨
  胀+等距+关臂。零修复授权维持。
- **D2 轮 6f 成对共模抑制=中心有效/膨胀有害；wall 点律全闭、DSC 余
  六格全是未在洁净场景采样的位置（2026-08-23，零素材桌面批；报告
  `~/.claude/plans/r30-materials/d2-paired-report.md`）**。**B 共模偏
  移坐实**：wall/DSC 网格/嵌套 B 位移均值同 ≈(−0.48,−0.47)px（0.69px
  RMS 一致），original/R1/R2 的 B 仅 0.19px（大蒙版探测行为不同）。
  **成对点律**：wall 九格全闭（free RMS 0.372/max 0.443px、+origin
  ≤0.98px）+DSC 嵌套九格全闭；**DSC 网格 G2/G7/G8/G9+R1/R2 仍 1.1-
  1.9px**——恰是只在 P26 上采过样的位置（上下边中/右侧列/大蒙
  版；墙面嵌套几何仅帧心/左边/左上角），edge_S 噪声前科在先。双集自
  由图心互差 ~5px、落 ≈base+(20,13)（距 base+(32,20) 13-14px）。**膨
  胀成对化反而变差**（boundary_geo DSC 18/19→11/18）：wall/嵌套 B 膨
  胀均值 ≈0 但 y 散布 0.13-0.22pp=噪声≫偏置——**教训入账：中心用成
  对差、膨胀用非成对**。**轮 7a 构建已落地（见上条）**（墙面两对孪生 d2wg/）：
  pair1=九格 M（轮 3 d2g 同几何）+pair2=R1/R2 原几何复刻（墙面无裁剪
  ⇒ R2 出幅警示消失）；预注册四图心约定（base/base+DCO/传感器心
  (4768,3168)/自由拟合）逐格点律预测+非成对膨胀三族预测；建成待用
  户导四张 9504×6336。零修复授权维持。
- **D2 轮 6e 图心=正号位移坐实但未闭合；R1 首次过闸；关臂共模偏移
  被点名为头号混杂（2026-08-23，零素材桌面批；报告
  `~/.claude/plans/r30-materials/d2-mapcentre-report.md`）**。
  **DefaultCropOrigin=(32,20) 双 RAW 首方证实**（decode.rs:3596-3616
  路径；两 RAW crop_area 同 @ (32,20)）；直测三号：正号 +origin
  (4784,3188) 压倒性优选（wall 点律 RMS 1.588→0.876px、DSC
  1.704→1.220px；负号 3.08/2.79px 被拒）、wall 角格残差被打到
  0.175-0.205px **但帧心反出 1.33px**；两数据集独立 2 参自由拟合图
  心**彼此仅差 2.737px**=共同位移倾向坐实（距 base+(32,20) 6.1-7.9px
  主要在 y=字面标签非完整答案）；DSC G8 顽固 2.45px。RR 六约定重开
  =canonical 半对角线仍最优。**尺寸账进展**：修正图心下 boundary_geo
  DSC 侧 **18/19（R1 −0.482 六轮来首次过 0.5pp 闸）**、boundary_mean
  clean RMS 0.115pp；wall 侧仍 5-6/9（0.25pp 级）。**批自点名最便宜
  下一判别=关臂共模偏移**：B−stored 全体 ≈半像素共同偏移+B 膨胀
  ≈−0.1pp 级共同偏置，而此前一切观测量都按 A−stored 计——帧心冲突与
  G8 顽固残差的头号嫌疑。**轮 6f 在飞**（零素材）：观测量全部改成对
  A−B 差（位移+膨胀同根一并成对化），重跑图心三号+自由拟合+全函数形
  族双数据集；点律 ≤1px 全闭+单族双集齐过=机理定案（预校正帧+按拍图
  +图心+函数形+成对系统差账）+修复义务。零修复授权维持。
...
- **D2 轮 6d 按拍元数据=前提证实、完整假设诚实失败；共性残差指向图
  ...
  提取证实按拍烘焙**：wall ARW 自带 mask-warp 角区更强（末 knot
  1.0367731 vs DSC 的 1.0312961、外四 knot 均差 +0.0043）、raw
  distortion 外区更低（−0.0038~−0.0066）；第三成像 exp_D（105mm）与
  两者都不同且趋势相反=capture/焦距专属元数据三点实证——**任何修复
  必须按拍读 RAW 自带图（引擎 lensmeta 现成路径）**。**量化不闭合**：
  角点残差 5.09→2.50px（解释一半）、edge 反涨 0.7→1.0px、函数形最
  优族 1/9→5/9（boundary_mean RMS 0.416→0.204、仍败 centre_L/
  edge_S/edge_L/corner_S 四格 max 0.320pp）；Test D 大头被解释
  （edge_L +0.208/+0.302、角 +0.35-0.37/+0.37-0.48）。方向问题未重
  开（逆图 edge/corner 反号 9.8/46.6px）。**共性新线索**：各配各
  knots 后两数据集角点残差同落 ~2.5-2.8px 级=共同未建模项，头号嫌
  疑=图心非几何帧心（R26 实证 ARW DefaultCropOrigin 全局位移
  (32,20)px 恰是此类共享残差的形状）。**轮 6e 在飞**（零素材）：
  DefaultCropOrigin 双 RAW 首方读取+直测双号约定、图心 2 参数自由拟
  合 vs 全部 21+ 中心位移测量（各配各 knots）、修正图心下重扫函数形
  族双数据集；闭合=机理定案（预校正帧+按拍图+图心+函数形）+修复义
  务，否则 honest partial。零修复授权维持。
  （2026-08-23，用户 d2w 双导出当日交付；报告
  `~/.claude/plans/r30-materials/d2-wall-verdict-report.md`（正本
  d2w/）；18 轮廓全 240/240、两 JPEG 首录哈希、导出=存储帧 9504×
  6336 零裁剪）**。**A 中心律部分过**：尺寸无关性漂亮通过（S→L 漂移
  0.019-0.070px）+死拟合心律再拒（角 L 差 11.1px）——但角点绝对位移
  (−21.30,−13.43) 距锁定预测 (−17.04,−10.65) 差 **5.1px**（P26
  上同残差仅 2.8px）。**B 等距性九格全过**（最差 0.258pp）：墙面
  edge_S 仅 0.173pp ⇒ 轮 5 的 0.855pp 裂缝**定性为暗区探测噪声**，
  等距分解保住。**C 三族锁定预测墙面全灭**（boundary_mean 1/9、
  conic 0/9、field 0/9，残差 0.4-0.76pp）——但与 D 纠缠：**D 场景无
  关性被响亮否定**：墙面 vs P26 同几何逐格膨胀差 +0.30~+0.48pp
  （edge_L+三角格全超 0.1-0.2pp 系统差）、角点位移差 ~2px。关臂
  RMS 0.491px 帧钉（centre_S 0.818px 略超 0.7 级=控制质量警示）。
  **主审上游假设（轮 6d 在飞验证）**：Sony 按拍烘焙畸变元数据（对
  焦距离改变 24-105 畸变强度）——锁定表用的是 P26 的 knots、墙
  面 RAW 自己的没提过；若墙面图更强则同号统一解释 (a) 角残差
  2.8→5.1px (b) 角膨胀 +0.37-0.48pp (c) edge_L +0.30pp。轮 6d=零素
  材桌面批：wall ARW 首方提取 knots→重算中心律+全部函数形族→若单一
  函数形在双数据集（各自 knots）齐过=机理定案+修复 dispatch 义务；
  另读 exp_D_ref.ARW（第三成像）knots 做按拍变化第三点。零修复授权
  维持。
  ...
  FE5745…1E26；ILCE-7RM4A+FE24-105 @24mm 同 P26 配置、四位置
...
- **🔒 v1.0.0 改为等 D2 闭合再发（用户拍板 2026-08-22）+ 会话暂停点
  ...
  （P35，judge Revise 成品如实标注、无存配方）+付费帽分毫不差
  ...
  差零手工缩放）→ apply 全 RAW 9504×6336；fit=P35 look error
  0.060→0.042 置信 0.747、P01 0.172→0.021 置信 0.870；四三联+
  联络表齐（合成 667KB）；**主审目验记录：P01 反推面板太阳品红
...
- **W2 展示批 1 验收通过+新规格 v2+三用户令落档（2026-08-22）**。
  **批 1 验收**（Codex 全程 dist exe 0.35.0+隔离 state，repo 零写
  亲验）：8 张批准照（用户逐张拍板「就这几张吧」：P35/09659/
  09709/00001、P01/0311/0598/0639——仅此八张可入公开 README）
  neutral+AI 全尺寸+8 对 1600px pair+P01 反推三联（look error
...
## 2026-09-02 归档批次（v1.2.4 清账批：旧横幅 + 「当前状态」46 条 + 标签定义 —— 逐字迁自 docs/ROADMAP.md，勿改写）

> 迁移依据：docs/ROADMAP.md 自 2026-09-02 起是发版台账，按既定惯例只留最新五条
> 「当前状态」条目与常驻参考；更早的条目、v0.16.1–v1.0.0 层积的旧横幅，以及
> 2026-08-18 入库的标签定义（其引用者本批一并迁出）逐字迁入本文件，一字未改。

### 旧横幅（docs/ROADMAP.md 头部引文，v0.16.1–v1.0.0 层积，2026-09-02 迁出）
...
### 「当前状态」条目 46 条（2026-08-24 … 2026-08-31，2026-09-02 迁出；主文件保留最新五条）

- **🎯 步7 R2 反推教义批已合并 main `4d852e2`（分支 `1cbc628` + 主审覆盖补丁 `f01753f`；2026-08-31，Opus 子代理交付、主模型亲核）**：有跨图对应场时 Atmosphere 的全局 EV/WB 改在**两帧共有内容**人口上解——目标侧剔掉「无任何可信源格映射其上」的生成内容、源侧镜像剔掉「内容已被目标替换」的像素，**两侧都剔**（合成对实证：真值 gr/gb 1.2181@0.00EV，整帧 0.911@+0.694、只剔目标 **1.945@−2.867**、只剔源 0.512@+3.593、两侧 1.216@+0.032——只剔目标比它要修的缺陷更糟）；二值切在 R2-lite 已公布的 `CONFIDENT_MATCH` 0.50；任一侧保留证据质量低于 `SHARED_POPULATION_MIN_RETENTION`（**直接写作 `EVIDENCE_RANGE_SURVIVAL_MIN`＝1−`DIVERGENCE_ZONE`＝0.35，非拷贝值**）即**拒绝**并说明（p37 保留 11%/7% 实证走拒绝路径）。**⚠ 验收硬线未达成，按用户裁定 2026-08-31「直接合并」合入**：岛屿底三分 R/B_lin −72.93%→−75.64%（要求正向）。**主审用自写脚本独立复算，与代理报告逐臂吻合 ±0.06pp**（neutral 0.8126／target 0.9759／基线 0.6937／R2 0.6892／神谕 0.7555／不动 WB 臂 +12.12%）。**不可达是实测的**：神谕人口（分割摘掉生成天空，无已发布仪器可识别）上界仅 **−34.99%**；真根因是**统计量**——同一对整帧逐通道**均值** R/B 比 **0.9997**（目标白平衡就是源的）对**中位数**比 **0.8257**，双峰画面上三通道各自独立的中位数取自不同子人口，其比值不是任何像素的颜色。**已立为计划步 #9「统计量批」，并按用户令「1.2.0 是最终版本、之后不允许任何改动」排在 v1.2.0 发版之前。****主审亲核实录**：八条具名测试＋M-R2-G′ 打红的三条既有测试第一方全绿；i18n 11 组全 0、字体 871/871；p36/p37/calib 全分辨率渲染两臂**逐字节同**（`b838ba4f…`/`aa8f3880…`/`9278c1ad…`，且 p36 与改名批台账已钉值跨批对上），岛屿对按报告所述尺寸差异；七对语料**六对配方逐字段零差异**、岛屿对恰动报告列出的五个字段。**主审三条亲手变异揪出两个覆盖缺口**（ADJ-1 保留门改 `||` 单边化、ADJ-2 目标掩码取自 `conf` 而非 `target_answered`）——批内八测全绿放行，成因是**每个夹具都让两侧保留率相等**；`f01753f` 用一个**非对称场**（处处可信、却只答目标一角＝岛屿 24%／p37 93% 推到极限）一次钉住两条，测试自证前提（源在门上、目标在门下）以防退化为又一个对称夹具，三条变异全红。**合并树门**：**1265(1253+12i)/23/159/2+2**、集差 **+9/−0 逐名**、clippy 0+0 双特性、check_docs --gates 26P/0F/1S（三处钉值 1256→1265 实测回填）；合并树亲手变异 M-ADJ-1 具名红后逐字节还原。**代理自报的两条过程发现均经主审复核属实**：一句从未跑过的变异断言（M-R2-G，实为等价变异，强形式 G′ 打红四条）已改写；报告初稿两个闭合数抄错已按磁盘重测改正。登记：岛屿对 `MAX_RATIONALE` 16 KiB 截断因新披露句加重（121→119 段），随统计量批一并处理。
- **🧩 清账 B1a 侧车样板已提交 `1d3e1fc`（2026-08-31，主审第一方）**：describe/embed/correspond 三脚本各自持有的 `log`/`die`/`model_dir`/`fetch_model`/`publish`（约 117 行写了三遍）逐份对拍后，差异只有三处——脚本自己的标签串、文档措辞、correspond 的逐文件 `makedirs`（严格超集：嵌套钉表得到子目录、扁平表多一次 `exist_ok` 空转）——没有一处支撑三份实现。新 `python/_sidecar.py` 是 `python/` 第三个共享模块（继 `_device.pick_device`、`denoise._fetch_verified`之后），并**代三脚本 import `_fetch_verified`**，下载-拒绝闸原地不动；每个脚本只留「哪个 MODEL、报哪个名」的四行委托，41 个调用点拼写零变化。**刻意留外两份且理由可测**：`denoise.log`（本模块 import denoise，反向即环）与 `segment.die`（实测 segment 导入后 `sys.modules` 里 numpy/torch/denoise 全无，而本模块经 denoise 拉入 numpy——为省两行把这份成本加到每次 `segment.py --help` 不划算）。**等价性实证而非论证**：双载对拍（改前备份 vs 树内）比 log 行、die 句与退出码、`model_dir`、每个 `_fetch_verified` 调用元组及顺序、所建目录、发布字节共 **24 门全同**，唯一差异即上述超集并已断言为超集。**四条 Rust 源不变量守卫随折叠重指向而非放宽**（`describe_sidecar_self_test_pins_every_file`、`the_sidecars_never_execute_unpinned_upstream_code`、`the_sidecars_publish_durably`、`the_siglip_tokenizer_files_are_digest_gated`）——守卫读的文本搬走了，留在旧文件即永久空转，比现在报红更糟；`_sidecar.py` 刻意**不进 SIDECARS 名册**（名册每条契约都在问 MODEL：≥3 个 digest、40 位 revision，而它一无所钉），禁令扫描纳入它、钉表遍历改看它、耐久检查按「谁拥有该侧车的 publish」选源以保证 fsync-before-replace 在同一文件内比较。新 `python/test_sidecar.py`（15 测试，循 `test_segment.py` 惯例，两个安装器都按 `test_*.py` 排除）钉五契约＋三绑定——describe.py 里写成 `_sidecar.log('embed', …)` 在评审里读着完全正常，只会表现为进度行署错名、或缓存目录钉到别的模型 revision。门＝电池三道并行 **1256(1244+12i)/23/159/2+2**、集差 **+0/−0 逐名**、clippy 0+0 双特性、check_docs --gates 26P/0F/1S（SKIP＝`AUTOSHADE_CENSUS_ROOT` 未设的库外语料普查）。**十条亲手变异全部具名红后按 sha256 逐字节还原**：新套件六条（扁平 makedirs／die 退 1／非原子 publish／全长 revision 目录／去掉字节帽余量／绑定串错）＋重指向守卫四条（绕过闸／子集遍历／去 fsync／把 `trust_remote_code` 种进共享模块——最后一条证明把它纳入禁令扫描不是装饰）。过程登记：首轮电池 4 条红正是这四条守卫，**它们如实抓住了折叠**（本批不是补测试而是修指向）；另登记本窗一次操作教训——shell 工作目录漂到 R2 worktree 导致一轮 check_docs 读了分支文档（1216 旧钉值），每次跨树命令须先 `git branch --show-current` 自证。
- **🧽 清账去重批五条已提交（2026-08-31，主审第一方；来源＝29 代理只读分诊工作流，19 条判定全 HOLDS 无一被驳）**：**B2c** 树里两份手写 SHA-256（describe 流式版＋eval 整消息版，各带 64 项常量表与各自已知答案测试；根因＝第二作者重推「不引 sha2」论证却未 grep `0x428a2f98`）→ 新 `src/sha256.rs` 收流式实现（整消息形是其退化），eval 改 use 并只留独有主张（一字节改动必改摘要＝陈旧侧车守卫前提），describe.rs 1048→889 行落回 CE 预算；**B2a** style.rs 三道字符截断各自手写「取 cap−1 补省略号」→ `bounded_chars` 单一规则，输出逐字节不变；**B2b** 三侧车各自实现「配置了脚本且文件存在」→ `lib.rs::sidecar_script_present`，三处一行委托；**D1** `lr_mask_unwarp_norm` 与 `linear_handle_unwarp_norm` 同 45 行同折叠守卫同 40 步二分仅差结点源 → `unwarp_norm_over` 参数化（表达式树与求值序不变＝渲染按构造逐字节同），「刻意分离」注释同批改写为按参数分离；**A12** 非命名问题而是真缺陷——校验器提示尾句写死「restraint on a **Brief** direction」，Hint/Direct 档也照发 → 改按本次 tier 生成，并给 AdherenceTier 单一拼写 `as_str`/`prompt_name`（消 pipeline 的 match 与 advisor 的 `{:?}` 两处独立拼写）+1 具名钉测（三档各验本档在、他档不在）。**五次亲手变异全部具名红后逐字节还原**：M-B2c 轮常量偏 1→FIPS 红；M-B2a 省略号挤出预算→描述边界测试红；M-B2b 去存在性→`look_build_refuses_without_the_sidecar_and_says_why` 红；M-D1 线性臂读径向样条→手柄输运守卫红；M-A12 尾句复常量→新钉测红（首版变异因占位符与参数数不匹配只是编译失败，重做为可编译变体才算数）。门＝电池三道并行 **1256(1244+12i)/23/159/2+2**、集差 **+3/−2 逐名**（−2＝随实现迁移的 `describe::…fips_180_4_vectors`→`sha256::…` 与收窄改名的 `eval::sha256_matches_the_published_vectors`→`a_one_byte_edit_changes_the_state_files_digest`，非能力丢失）、clippy 0+0、check_docs --gates 27P/0F/0S（三处钉值 1255→1256 实测回填）。过程登记：首版 A12 测试照抄邻测骨架用错签名（`Intent` 不存在、`build_verify_prompt` 取 4 参），由并行 clippy 道以编译错误抓出——并行道的价值实例。同批按分诊报告结案五条**无需改码**项：**A4 关闭**＝已被 `9097319` 修好（生产路径零 unwrap，写手走 ensure_parent+durable_write）；**A9 转观察**＝增长主张不成立（单文件在 store_root 且有容量上限即回收规则），真症状是跨库交替构建的保留抖动；**A11 转观察**＝169 例索引实测恰 4 个八位机身计数名各由 2 张不同目录照片承担、每个键控面已按路径消歧有测试，仅披露文本可能同名；**D9 转观察**＝纹理比统计量本就无法分离生成云修正（0.961 落在接受区 0.730–1.918 内），矛盾已钉在夹具+披露+ARCHITECTURE，动阈值即误拒或粉饰；**D11 转观察**＝台账用词「look-vivid」全树零命中，实质议题是 tags_from_scores 未减库均值，一行算术但会重写现有索引每个标签，须随索引版本同做。**清账进度 15/41**（余 26＝A 组 9/B 组 1/D 组 8/E 组 5，其中 E 组属发版步）。
- **🧹 清账 A3+A8 已提交（2026-08-31，主审第一方）**：**A3** denoise 停机测试的挂钟上界 `elapsed < 5 s` 换成机制下界（`elapsed >= 100 ms` 截止真已到期）——旧断言测的是调度器（spawn+两道 2 s 排空宽限只剩约 1 s 余量），负载下假红；kill 的结构性由拒绝文本与被释放的 claim 证，同类挂钟上界全仓仅此一处（advisor SSE 那条是 0.2 s vs 6 s 相隔 30× 的判别器，保留）。**A8** `style-query` 的 terms 行补 hubness 披露三态（`,hub=<均值>` 命名被减去的值／裸 `,hub` 为修正在场但该对无余弦／前词表索引不打印，逐字节同修正问世前），兑现 `txt_hub_corrected` 文档「读 terms 的人会被告知」的承诺。+1 具名测试（CLI 22→23），两亲手变异具名红（M-A8 撤披露→钉测红；M-A3 截止永不武装→下界断言红，实测 20.4 ms）。门＝电池三道并行 **1255(1243+12i)/23/159/2+2** 集差 +1/−0 逐名、clippy 0+0、check_docs --gates 27P/0F/0S（三处 CLI 钉值 22→23 按实测回填）。
- **🔑 C2 两处旧存储名迁移批已提交（2026-08-31，主审第一方；用户裁定「直接改，不接受旧名字」推翻 R1 期「永久接受」）**：eframe 偏好＝`STORAGE_KEY` 全平台统一 "AutoShade"（cfg 分叉删除），启动时 `adopt_pre_rename_prefs` 按店教义收养 `%APPDATA%\Autoshop`（整体 rename+单行披露；失败＝本次沿用旧键零重置、下次重试；双存＝新键生效旧目录不动；macOS 承店 opt-out；`AUTOSHADE_DATA_DIR` 沙箱跑绝不碰真实目录——persistence_path 本就整体覆盖）；导出登记簿＝`EXPORT_REGISTRY_DIR` 改 ".autoshade-export-registry"，`adopted_export_registry_root` 首用时整体 rename 旧命名空间（claim 逐字节随迁＝交付后缀不变；rename 败且新目录不存在→本调用沿用旧命名空间；披露自限＝迁移后旧目录即消失，至多一次）。+3 具名测试（登记簿收养带 claim、prefs 四臂 Nothing/Migrated/KeptBoth/FellBack、键对+平台门一决定），3479 拼写钉改双拼写对钉。两亲手变异具名红（M-C2-A 收养禁用→登记簿测试红；M-C2-B 假迁移不搬→prefs 测试红），还原以 cmp 逐字节+标记双证。门＝电池五道并行 **1255(1243+12i)/22/159/2+2** 集差 +3/−0 逐名、clippy 0+0、check_docs --gates 27P/0F/0S；ARCHITECTURE §4.11 教义句重写（迁移取代「Windows 保旧键」）、v1.2.0 义务清单开列（首启迁移披露条）。顺带清 R2 登记小项：仓库自带 cwd 配置文件 `git mv` 为 `autoshade.local.json`（代码本就优先读新名，别名告警随之消失）。
- **🔒 C1 照片文件名清源已提交（2026-08-31，主审第一方，用户裁定「全部别名化 P-序号」）**：src/ 全量普查真名 stem **124 处/12 文件**（原挂账「src 注释 19 处」系粗查低估——本次含代码字面量与夹具串），**121 处别名化为稳定 P 码**（23 个 stem 复用既有解码环 P04-P34，13 个新增 P40-P52；别名↔原名对照表私存用户目录不入库），3 处 DSC00002 为通用计数器夹具**刻意保留**（与 DSC00001/3/9、DSC00042 及合成族 DSC0001-4/DSC00099 同类：任何 Sony 相册都产生同名、无归属注记，撞库属命名空间固有，拒改理由载解码环注）；xmp.rs 一处含日期地点的真实库路径改述为别名。两个 env 门控真夹具套件（`AUTOSHADE_MB_FIXTURES` 七侧车、`AUTOSHADE_MB_SAMPLE_ROOT` 笔刷样本）**本机夹具文件同日改名 P 码**（m-b 7 份 .xmp.txt、mb-sample 8 份），改名前后主审亲跑同数字双绿（七侧车 3 non-neutral B2；样本 54/4/18 表、3043 dabs）＝磁盘契约零破坏。门：电池并行五道 **1254(1242+12i)/22/157/2+2**、**集差逐名 +0/−0**（1437 具名全同＝纯改名零行为变更）、clippy 0+0、check_docs --gates 27P/0F/0S。**同批裁定落档**：C3 旧名占位仓**风险主张撤销**（GitHub 仓库名在所有者命名空间下、第三方无法创建 skymanbp/autoshop，用户质疑属实，r2-checklist §1 已勘误，不建占位仓）；C4 公开文档场景名一律用 P 码（两个中文场景名皆弃）；C2 两处旧名（eframe 存储键 %APPDATA%\Autoshop、.autoshop-export-registry）用户裁「直接改」→读旧写新迁移批待排。
- **🎨 色彩质量波·批3 已主审亲核合并 main（2026-08-31 merge `5cef7e1`，分支 advisor-batch3 `2a415a5`，7 文件 +826/−57，零冲突）——步 3 四批合并全部收口**：G2=colour_guardrail_pair 从语料数字铸带（色调对 ±75/±55、HSL ±20-40、色轮总量 40-80、曲线 7-9 点、主深 15-30/通道深 8-15 @ Committed 1.0；0.5 与 0.65 经 normalise 逐字节同）+中性逃生口按强度模板化（1.0 档须在 rationale 自辩）+评委 rubric 正向色彩项+校验器色彩/曲线单调项；G3=HABIT_SLIDERS 10→11 追加 hue、shapes_colour 含 saturation+hue、三名额规则色彩轴保底一席、蒙版色彩短语配数字。**亲核实录**：最终合并树电池 **1254(1242+12i)/22/157/2+2** 集差 **+9/−0 逐名**与批报告单一致、clippy 0+0、check_docs 27P/0F/0S、i18n 全 0、字体 864/864；亲手变异「复制 clause」→local_work_note_fits_its_bound 具名红（27P/1F）还原净＝其收窄断言（数 saturation -100 短语非裸词）拦真重复实证；批2 跨批绊线 every_habit_slider_is_addressable 在合并树保持绿＝hue 可寻址；定向抽查=两处 MAX_CURVE_POINTS 引用自纠核实（EditRecipe::clamp 局部 const）、中转残留/照片名/密钥三扫 0、宽度迁移零版本跳（habit_mean 宽容 serde+SLIDER_FLOOR 静默未测列+11 宽夹具改由 N_HABIT_SLIDERS+1 生成）。活体 8 次付费调用（恰顶帽）：曲线非空 4/6（语料 0/55）、点数 8-9、HSL 最强带 29/30、色轮 49/52 达标；**蒙版 hue 0/6 如实未达成**（G3 习惯半程无活体仅单元测试）、评委方向 n=2 样本不足自认。**登记**：三代理并发 GUI 电池共享按用户 store 根致中途 4 红（终趟 151P/0F，对策=电池一律显式 AUTOSHADE_DATA_DIR 临时目录，入清账）；批3 每用户存储曾钉死中转 403 未计费后改官方 API。
...
> 以下「标签定义」节自 docs/ROADMAP.md 逐字迁出（2026-08-18 入库，2026-09-02 迁出）；
> 它自带二级标题，故置于本归档批次之末。
