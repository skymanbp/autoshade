...
| **M-C** | `f944ef3` 那 147 张 RAW+xmp 对照集是否还在、路径确认 | ✅ R25 全量重跑（数字见 v0.31.0 条）；对照集本体经 R27 Batch-6 复核仍在（`D:/Photography/Raw`，3678 RAW → 恰 147 对，表头与 R25 逐字相同）。**HEAD 重基线两次中断未落地**（83/147 撞 commit 内存墙 + 分块重试被用户重启截断）；用户 2026-08-19 拍板改**中转站 API + 并行**在 R27 内重跑，落地前 R25 仍为唯一参照 |
...
- **v0.31.1（2026-08-18，证据修正补丁 —— 三条「锁定假设」被网络取证证伪后的
  ...
  **← R27 Batch-6 处置（L-31 未结清）**：这条不可比性连同另外两条（R23 提示词
  全档重写、v0.32.0 的 (32,20) 像素位移）拟由同一份 HEAD 全量重跑一次性作废；
  Batch-6 两次尝试均中断（83/147 撞 commit 内存墙 + 分块重试被用户重启截断），
  用户 2026-08-19 拍板改**中转站 API + 并行**在 R27 内重跑。**落地前 R25 那份
  仍是唯一可引参照**，L-31 保持在册。
...
- **v0.29.0 RELEASED（2026-08-17，第二十三轮=17 条反馈三轮清账之 R23：AI 大
  ...
  **#3/#16**（反推大修：诚实性批=置信族一次标定两端具名+ratio 拒绝披露+终局
  ...
  （提示词全档重写），147 张对照集重基线待 M-C——**R27 Batch-6 两次重跑尝试均
  中断（commit 内存墙 + 重启），L-31 仍在册**；用户 2026-08-19 拍板改中转站
  API + 并行在 R27 内落地，落地前 R25 基线仍为唯一参照。**行为变更（发版说明）**：
