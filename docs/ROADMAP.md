...
## 第十二轮计划（🔒 用户锁定 2026-08-09 —— 预定 v0.23.2）
...
- **第十二轮·阶段 1–3 全清：在册 15 簇归零 + Codex 簇群复审闭合（2026-08-11，
  ...
  **AP 快捷键**：+/−/0/1 缩放四键（1:1 经 zoom_one_to_one 每帧孪生，
  按钮与键同源）；Ctrl+C/V 镜像复制配方/粘贴到选中（tier C，文本框
  保留 egui 剪贴板）；↑/↓ 图库走图（controls_rect 悬停让位滑杆微调，
  本处理器先跑、无条件消费会吃掉 nudge）；滑杆右键=双击重置；备忘表
  30→33 行；头less 真帧驱动测试（步进/12× 顶格/适配回中/1:1 跳转/
  无照惰性）。
  ...
  **真机验证（PrintWindow 后台捕获，全程不碰前台/光标——用户当时在
  ...
  门（每批全过）：测试 371+5+54；等集累计 **+37/−0**（快照
  current_names_AB→AQ）；clippy 0；i18n 审计 9×0（zh 至 603 条）；字体
