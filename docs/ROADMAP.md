...
- **R12 双路全仓 debug 修复潮（2026-08-04，用户令"你自己和codex进行两路
  ...
  - R2 双路复扫（对七个修复提交自身的对抗复审）进行中→收敛后推送+发布
    v0.16.0。发布说明须披露：v0.15.0 的预览/修饰母版/基调估计整体
    +0.5 偏白；该期首次打开的照片基调曲线来自偏白帧（重开会重估）。
...
- 待做候选（未开工）：
  - **InPlace 修饰母版无移除通道**（R12 批 68 记录）：健康的 heal/clone 母版在
    GUI 与 web 都没有"分离/移除"手势——GUI 清除守卫要求画布无 origin，而只有
    反推（仅 Generated 可用）或母版**加载失败**才产生无 origin 画布（坏母版反
    而可移除、好母版不可）；web 端中性保存自批 65 起改为保留母版并在响应中披
    露。需要一个显式"移除修饰"动作（确认 + pixels.json 清理 + 画布回源）。
