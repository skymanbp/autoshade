...
- **R17 IN TREE 未发版（2026-08-12，第十七轮=反推灰纱根修·证据识别自警）** —
  ...
  **真实对终态**：复合解 0.0947→**0.0267**（sat 52.6 脱顶格），渲染亮度分位
  ...
  与 ARCHITECTURE 同步）。**有据不做/在册**：①分区 ≥50% 门槛**立案待查**
  （用户拍板：本对 sky 0.012→0.009=25% 真实改善被弃、land 0.036→0.047 拒得
  对；全局修复后分区残差基线已变，待真实 zoned 反推重评）；②"共享类当第三
  臂证据"（Opus 提议）有据不采：共享类按定义丢失迁出带亮度域（本对将零亮天
  证据）且继承对齐依赖，两个真实坏对全量回退实测更优；③1.75× 份额门与误预
  测门的关系（均匀 2× 膨胀会被份额门误回退而误预测≈0）登记为后续取舍；④对
  称换血为指标结构盲点，已在 doc 披露。**历史注**：2026-07-09 批次 #2-A 所
  记 tone_cdf_pair 两条件与旋转普查"两端可见 chroma≥0.04"自本轮起被三条件
  +可见性豁免取代（历史条目保持原文）。门：**437 lib（+5：毒对蒸馏/良性膨
  胀/canyon 匹配/haze 回退钉/结密度钉）+ 7 CLI + 72 GUI、clippy 0、env 复现
  全绿**。**未发版**——桌面 exe 仍是 v0.25.0 旧解算器，目验需先重编译
  （或随下次发版）。
