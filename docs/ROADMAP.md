...
> 开工。更新于 2026-08-21（**版本号已推到 v0.35.0，发版链未跑**——已发布的
> 最新 tag 仍是 `v0.34.0`；R29 的七项渲染/兼容硬变更与发版说明置顶义务
> 清单见「当前状态」首条；以下横幅为
...
- **R29 全量文档 sweep 批已落地（2026-08-21，未发版，本批）：版本推到
  ...
  **⚠ 本批改了一条文档门，理由必须留档**：`check_docs` 的
  ...
  「上一次发版 9/9，每版重跑重记」。**发版链据此欠三件**（本批无法代跑，
  夹具语料不在仓内）：`AUTOSHOP_RAW_ZOO`、`AUTOSHOP_MB_FIXTURES` 两套
  env 门要在 tag 前实跑并把数字记进台账与 README；`AUTOSHOP_LR_PROBE_FIXTURES`
  16/16 已在 `a841ec3`（＝本批的代码状态，本批只改注释）实跑，可沿用但
  按 L-29 纪律仍应复跑。README 的资产表仍是 v0.34.0 的字节数与 sha256，
  已就地标明「v0.35.0 的在 tag 时补」——发版链回下载逐位 cmp 后替换。
