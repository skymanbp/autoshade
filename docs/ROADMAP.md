...
- **R11 后修复潮一（2026-08-04，用户四项拍板后开工）**——四项决策：成品
  ...
  - 终态：**44 提交未推送**；168 lib + 1 cli + 10 gui 双配置全绿；
    clippy 双零；i18n 436/464/0/0；JS 解析过。真机验收点（WB 批）：
    开 RAW 见"as shot ≈ 相机实拍 K"、勾自定义 WB 画面不跳、Temp 数值与
    LR 同片读数同义、旧存档观感逐字节不变、旧 XMP-only 存档渲染不变；
    （A4/A6 批）：窗口缩放后 web 选区框不漂、断母版链导出报错而非假成功、
    损坏 XMP 打开即见披露。待办：A7 缓冲7 / D13 键位23 / U14 弱测试。
