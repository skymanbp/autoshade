...
| 6 | `Exposure2012` numeric range bound (decimal type verified; ±5.0 not doc-confirmed) | **OPEN.** The GLOBAL bound is still doc-unconfirmed. What R27 did settle is its LOCAL twin: `crs:LocalExposure2012="1"` = +4.00 EV exactly, measured on `_DSC9593.xmp` (V2_PLAN §7 item 2), and the import gate sits one stop wider at ±5 on purpose (`src/xmp.rs:5026`). **New plan:** the same one-frame experiment on the GLOBAL slider — push Exposure to +5.00 and to −5.00, Ctrl+S, read the two values — rides the next Lightroom session. |
...
*v0.33.0 (R27 Batch-1b) correction to the heading's inference.* The census itself stands — the library really is one camera family — but **“→ one decode target” stopped being true of the PROGRAM on 2026-08-19**: the accepted-input set is 24 camera-RAW extensions (`decode::RAW_EXTS`, [src/decode.rs:120](../src/decode.rs)) plus 8 already-baked raster extensions (`pipeline::BAKED_EXTS`, [src/pipeline.rs:4120](../src/pipeline.rs)), and rawler dispatches on file CONTENT, never on the extension, so that list names what is worth handing it rather than what it can read. Read §9.1–9.2 as the shape of the EVAL corpus, not of the input gate.
...
- **Eval consequence, and why it is deliberately left to a re-baseline.** The exclusion above — "`Texture` is out-of-scope, not AI error" — was correct in M1 and is now wrong for `Texture`, which the engine renders. The eval ruler is derived from the control registry (R23-1), so a key changing tier changes the score; scores are ALREADY not comparable across R23 (the prompts were rewritten wholesale). ~~The R25 full re-baseline over the 147-pair set therefore IS the new reference~~ **— and by 2026-08-19 the R25 baseline had itself gone stale on three further axes (v0.31.1's `Sharpness` 1:1 correction, v0.32.0's (32, 20) px render shift, and R27's mask-import work). R27 Batch-6 attempted the HEAD re-run twice and both attempts died mid-run (photo 83/147: the machine hit a commit-memory wall — root-caused and fixed by Batch-7's memory-budgeted `--jobs`; the chunked retry was cut short by a reboot the user ordered). **The third attempt LANDED 2026-08-19**: HEAD `f43dd85`, relay endpoint (gpt-5.6-sol) + `--jobs 3`, 147/147 with zero fallbacks on both streams, ~38 min — `~/.claude/plans/r27-materials/mc-eval-147-r27.txt` is the reference now, R25 demoted to history. Headlines: gap 15.5%; mask-import refusals 2.35 → 0.05 per photo (the Batch-4 root fix, proven on the full corpus); whites bias +22.55 → +16.20, blacks −14.18 → −10.87** — and re-deriving old numbers against the new run would be arithmetic on two different rulers.
  - **R27 Batch-7 diagnosed the wall the re-run died on, and it is not a leak.** Two first-party measurements, no paid call: the `#[ignore]`d estimator probe in `eval.rs` decoded all 147 pairs in 44.77 s with commit charge flat across the run (quartile means 112 / 127 / 115 / 122 MB, peak WS 254.5 MB), and a scratchpad probe replaying `produce_recipe`'s non-network half read the process's own `PeakPagefileUsage` between photos: **decode half 151 MB peak, calibration half (`pipeline::photo_base_knots` → `render::render_to_image`) 1771 MB peak**, and over 11 consecutive photos that high-water mark moved +1.0 MB total while resident commit between photos held at 12.2–12.6 MB. So one photo's pass PEAKS at ~1.77 GB — nearly all of it the full-frame oriented f32 demosaic, which `render_to_image` produces BEFORE it applies `max_edge` (render.rs:266-272), so `photo_base_knots`' ≤2048 cap bounds the develop stages but not the peak — and nothing accumulates. The re-run therefore needs a memory-BUDGETED pool, which is what `--jobs` now is (`jobs::PER_PHOTO_PEAK_COMMIT_MB`); it does not need a leak fix, because there is no leak. Not measured, and registered as such: the `claude` verifier subprocess this loop spawns per photo is a Node runtime whose own commit is outside that constant.
