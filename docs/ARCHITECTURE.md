...
| Third-party parser panics | **Named error**, process survives (`decode::guard_parser_panic`) — and the crash modal knows the difference (`panic_guard`) | The GUI has wrapped workers in `catch_unwind` since v0.22; the CLI had nothing, so one malformed file killed a whole `batch` run. The GUI panic HOOK then ran INSIDE both guards and could not tell contained from fatal, so a single unreadable photo raised "AutoShade hit an internal error and must close" over an app that kept running and went on to report that file's own error. `panic_guard`'s thread-local depth — entered by both guards, restored on the unwinding path — is what the hook now reads to keep its own claim-matches-outcome rule; the report is still written either way, headlined "recovered from" instead of "crashed" |
...
disabled until it lands). R30 gave that entry its own sub-area and gated what
CONSUMES a library, not what shapes one: while the Style slider sits at 0,
`(req.style > 0.0).then(load_effective)` in `pipeline.rs` opens no index, so the
two switches that do nothing but feed an analysis — the reference-photo switch
and **Use look library** — are drawn disabled with the reason above them.
Everything else stays live at every Style value: the folder pickers, both Build
buttons, and the retrieval-engine rung, whose **Use SigLIP 2 look embedding**
and **Describe looks…** switches `actions.rs` resolves when it starts a build
and which therefore decide what that build computes, not only what a query is
matched on.
